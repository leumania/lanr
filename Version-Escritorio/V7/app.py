...
def init_db():
    ...
    unidades_iniciales = [
        ...
        ('JUEGO', 'REQ'), ('GLB', 'REQ'), ('KG', 'AMBOS'), ('GLB', 'SP'), ('DIA', 'SP'),
        ('HE', 'SP'), ('GAL', 'SP'), ('HM', 'SP'),
        ('PTO', 'SP'), ('MES', 'SP'), ('M3', 'SP')
    ]
    ...
    for columna, definicion in (
        ...
        if columna not in columnas_gestion:
            c.execute(f'ALTER TABLE gestion_logistica ADD COLUMN {columna} {definicion}')
    ...
    columnas_tesoreria = [row['name'] for row in c.execute("PRAGMA table_info(solicitudes_tesoreria)").fetchall()]
    for columna, definicion in (
        ...
        if columna not in columnas_tesoreria:
            c.execute(f'ALTER TABLE solicitudes_tesoreria ADD COLUMN {columna} {definicion}')
...
def generate_payment_request_pdf(tid):
    c=db()
    t=c.execute('''SELECT t.*,u.full_name creador_nombre FROM tramites t JOIN users u ON u.id=t.creador WHERE t.id=?''',(tid,)).fetchone()
    if not t or t['tipo']!='SP': c.close(); abort(404)
    items=c.execute('SELECT * FROM items WHERE tramite_id=? ORDER BY nro',(tid,)).fetchall()
    aps=c.execute('''SELECT a.*,u.full_name nombre FROM approvals a JOIN users u ON u.id=a.usuario_id WHERE a.tramite_id=? ORDER BY a.id''',(tid,)).fetchall()
    modalidades=c.execute('SELECT modalidad FROM sp_modalidades WHERE tramite_id=? ORDER BY id',(tid,)).fetchall()
    cuentas=c.execute('SELECT banco,cuenta_cci FROM sp_cuentas WHERE tramite_id=? ORDER BY id',(tid,)).fetchall()
    comps=c.execute('SELECT * FROM sp_comprobantes WHERE tramite_id=? ORDER BY orden,id',(tid,)).fetchall()
    adjuntos=c.execute('SELECT * FROM attachments WHERE tramite_id=? ORDER BY orden,id',(tid,)).fetchall()
    g=c.execute('SELECT * FROM gestion_sp WHERE tramite_id=?',(tid,)).fetchone()
    admin_name=''
    if g and g['asignado_por']:
        r=c.execute('SELECT full_name FROM users WHERE id=?',(g['asignado_por'],)).fetchone(); admin_name=r['full_name'] if r else ''
    gg_name=''
    if g and g['conformidad_gg']:
        r=c.execute("SELECT full_name FROM users WHERE role='Gerencia General' AND active=1 ORDER BY id LIMIT 1").fetchone(); gg_name=r['full_name'] if r else ''
    c.close()
    ...
    title=ParagraphStyle('LANRTitle',parent=styles['Heading1'],fontName='Helvetica-Bold',fontSize=12,leading=14,alignment=TA_CENTER,textColor=colors.HexColor('#17365D'))
    small=ParagraphStyle('LANRSmall',parent=styles['BodyText'],fontSize=7,leading=9)
    tiny=ParagraphStyle('LANRTiny',parent=styles['BodyText'],fontSize=6.5,leading=8)
    doc=SimpleDocTemplate(base,pagesize=A4,rightMargin=16*mm,leftMargin=16*mm,topMargin=14*mm,bottomMargin=14*mm)
    story=[]
    logo=os.path.join(BASE,'static','logo-lanr.jpeg')
    if os.path.exists(logo): story.append(Image(logo,width=36*mm,height=14*mm))
    story.append(Paragraph('SOLICITUD DE PAGO',title))
    story.append(Paragraph(f'N° {t["numero"]}',ParagraphStyle('num',parent=title,fontSize=9)))
    story.append(Spacer(1,4*mm))
    story.append(Paragraph(str(t['proyecto'] or ''),small)); story.append(Spacer(1,3*mm))

    info=[['TIPO DE SOLICITUD',t['subtipo'] or ''],['BENEFICIARIO',t['beneficiario'] or '']]
    if t['dni_ruc']: info.append(['DNI / RUC',t['dni_ruc']])
    if modalidades: info.append(['MODALIDAD DE PAGO',' / '.join(r['modalidad'] for r in modalidades)])
    for ct in cuentas:
        if ct['banco']: info.append(['BANCO',f"{ct['banco']}{' · '+ct['cuenta_cci'] if ct['cuenta_cci'] else ''}"])
    if t['responsable']: info.append(['RESPONSABLE',t['responsable']])
    if t['celular']: info.append(['N° CELULAR',t['celular']])
    info.append(['FECHA',t['fecha'] or ''])
    if t['fecha_limite_pago']: info.append(['FECHA LÍMITE DE PAGO',t['fecha_limite_pago']])
    ti=Table([[Paragraph(str(a),tiny),Paragraph(str(b),tiny)] for a,b in info],colWidths=[44*mm,118*mm])
    ti.setStyle(TableStyle([('GRID',(0,0),(-1,-1),0.4,colors.HexColor('#9CA3AF')),('BACKGROUND',(0,0),(0,-1),colors.HexColor('#E5E7EB')),('FONTNAME',(0,0),(0,-1),'Helvetica-Bold'),('VALIGN',(0,0),(-1,-1),'MIDDLE'),('LEFTPADDING',(0,0),(-1,-1),4),('RIGHTPADDING',(0,0),(-1,-1),4),('TOPPADDING',(0,0),(-1,-1),3),('BOTTOMPADDING',(0,0),(-1,-1),3)])); story.append(ti); story.append(Spacer(1,4*mm))

    data=[['ÍTEM','CONCEPTO','N° DESP.','UND','CANT.','C. UNIT.','MONTO']]
    negrows=[]
    for idx,i in enumerate(items,1):
        q=float(i['cantidad'] or 0); cost=float(i['costo'] or 0); m=float(i['monto'] or 0)
        data.append([str(i['nro'] or idx),Paragraph(str(i['descripcion'] or ''),tiny),i['nro_despacho'] or '',i['unidad'] or '',f'{q:g}',f'{moneda} {cost:,.2f}',f'{moneda} {m:,.2f}'])
        if q<0: negrows.append(idx)
    tb=Table(data,colWidths=[10*mm,65*mm,18*mm,13*mm,16*mm,24*mm,25*mm],repeatRows=1)
    st=[('GRID',(0,0),(-1,-1),0.35,colors.HexColor('#9CA3AF')),('BACKGROUND',(0,0),(-1,0),colors.HexColor('#D9EAF7')),('FONTNAME',(0,0),(-1,0),'Helvetica-Bold'),('FONTSIZE',(0,0),(-1,-1),6.2),('VALIGN',(0,0),(-1,-1),'MIDDLE'),('ALIGN',(0,0),(0,-1),'CENTER'),('ALIGN',(2,1),(-1,-1),'CENTER')]
    for r in negrows: st.append(('TEXTCOLOR',(4,r),(-1,r),colors.red))
    tb.setStyle(TableStyle(st)); story.append(tb); story.append(Spacer(1,3*mm))

    totals=[['TOTAL',f'{moneda} {total:,.2f}']]
    if float(t['amortizacion'] or 0)>0:
        totals.append(['SOLICITUD DE PAGO (AMORTIZACIÓN)',f'{moneda} {float(t["amortizacion"]):,.2f}']); totals.append(['SALDO PENDIENTE DE PAGO',f'{moneda} {float(t["saldo_pendiente"] or 0):,.2f}'])
    totals.append(['ABONO',f'{moneda} {float(t["abono"] or 0):,.2f}'])
    tt=Table(totals,colWidths=[110*mm,52*mm]); tt.setStyle(TableStyle([('GRID',(0,0),(-1,-1),0.4,colors.HexColor('#9CA3AF')),('BACKGROUND',(0,0),(0,-1),colors.HexColor('#E5E7EB')),('FONTNAME',(0,0),(-1,-1),'Helvetica-Bold'),('ALIGN',(1,0),(1,-1),'RIGHT'),('FONTSIZE',(0,0),(-1,-1),7)])); story.append(tt)
    if t['observaciones']: story.extend([Spacer(1,3*mm),Paragraph('<b>OBSERVACIONES:</b> '+str(t['observaciones']),small)])
    if comps:
        story.extend([Spacer(1,3*mm),Paragraph('<b>COMPROBANTES REGISTRADOS</b>',small)])
        for cp in comps:
            label=' · '.join([x for x in [cp['tipo'],cp['numero']] if x]) or 'Comprobante'; story.append(Paragraph('• '+label,tiny))
    story.append(Spacer(1,8*mm))
    art=next((a for a in aps if a['rol']=='Gerencia de Obra'),None)
    sig=[['GERENCIA DE OBRA','ADMINISTRACIÓN','GERENCIA GENERAL'],[art['nombre'] if art and art['aprobado'] else 'PENDIENTE',admin_name or 'PENDIENTE',gg_name or 'PENDIENTE']]
    ts=Table(sig,colWidths=[54*mm]*3,rowHeights=[8*mm,14*mm]); ts.setStyle(TableStyle([('GRID',(0,0),(-1,-1),0.4,colors.HexColor('#9CA3AF')),('BACKGROUND',(0,0),(-1,0),colors.HexColor('#E5E7EB')),('FONTNAME',(0,0),(-1,0),'Helvetica-Bold'),('ALIGN',(0,0),(-1,-1),'CENTER'),('VALIGN',(0,0),(-1,-1),'MIDDLE'),('FONTSIZE',(0,0),(-1,-1),6.5)])); story.append(ts)
    doc.build(story); base.seek(0)

    writer=PdfWriter(); br=PdfReader(base)
    for pg in br.pages: writer.add_page(pg)
    anexos=[]
    for cp in comps:
        if cp['nombre_archivo']: anexos.append(cp['nombre_archivo'])
    for a in adjuntos: anexos.append(a['nombre_archivo'])
    for name in anexos:
        ...
        try:
            if name.lower().endswith('.pdf'):
                rr=PdfReader(path)
                for pg in rr.pages: writer.add_page(pg)
            elif name.lower().endswith(('.png','.jpg','.jpeg','.webp')):
                ib=io.BytesIO(); cc=canvas.Canvas(ib,pagesize=A4); W,H=A4
                img=ImageReader(path); iw,ih=img.getSize(); scale=min((W-30*mm)/iw,(H-30*mm)/ih); dw,dh=iw*scale,ih*scale
                cc.drawImage(img,(W-dw)/2,(H-dh)/2,width=dw,height=dh,preserveAspectRatio=True,mask='auto'); cc.save(); ib.seek(0); rr=PdfReader(ib)
                for pg in rr.pages: writer.add_page(pg)
        except Exception as e: print('No se pudo anexar a SP:',path,e)
...
def ensure_integral_schema():
    ...
    order_cols={r['name'] for r in c.execute("PRAGMA table_info(ordenes)").fetchall()}
    for col, ddl in (
        ('vb_gerencia_obra_por','INTEGER'),
        ('vb_gerencia_obra_fecha','TEXT'),
        ('vb_administracion_por','INTEGER'),
...
def home():
    ...
    if role in ('Gerencia de Obra','Control y Planeamiento','Sistemas'):
        ...
        activos = c.execute(
            ...
            AND (
                t.estado != 'Pendiente de mi revisión'
                OR t.creador = ?
            )
            ''',
            (u['id'],)
...
def get_active_units(c):
    ...
    req_units = [r['abreviatura'] for r in rows if r['uso'] in ('REQ', 'AMBOS')]
...
def create_requirement():
    ...
    year = fecha[:4]
    try:
        numero = compose_form_number('REQ', request.form.get('numero_correlativo'), year)
    ...
    for i, descripcion in enumerate(desc):
        ...
        prioridad_item = prioridades[i].strip() if i < len(prioridades) else 'Normal'
        if prioridad_item not in ('Normal', 'Prioritario', 'Urgente'):
            prioridad_item = 'Normal'
        fecha_requerida_item = fechas_req[i].strip() if i < len(fechas_req) else ''
    ...
    for seccion, descripcion, unidad, cantidad, stock_actual, justificacion, prioridad_item, fecha_requerida_item, item_key in req_items:
        ...
        # Imágenes opcionales asociadas exactamente a este ítem.
        for archivo in request.files.getlist(f'imagenes_{item_key}'):
            if not archivo or not archivo.filename:
                continue
            ...
            c.execute(
                ...
                (tid,item_key,nombre_seguro,guardado,datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            )
    ...
    documentos_req = [f for f in request.files.getlist('documentos') if f and f.filename]
    if documentos_req:
...
def create_payment_request():
    u = user()
    if not can_create_payment_request(u): abort(403)
    obra = active_project()
    if not obra:
        flash('No tienes una obra activa asignada.', 'danger'); return redirect(url_for('home'))
    ...
    if subtipo not in ('Proveedor persona natural','Proveedor persona jurídica','Cuarta categoría / Recibo por Honorarios','Planillas'):
        flash('Seleccione un tipo de solicitud válido.','danger'); return redirect(url_for('new_payment_request'))
    if not fecha or not beneficiario:
        flash('Fecha y beneficiario son obligatorios.','danger'); return redirect(url_for('new_payment_request'))
    try:
        numero = compose_form_number('SP', request.form.get('numero_correlativo'), fecha[:4])
    except ValueError as exc:
        flash(str(exc),'warning'); return redirect(url_for('new_payment_request'))

    moneda = request.form.get('moneda','PEN')
    if moneda not in ('PEN','USD'): moneda='PEN'
    ...
    for i,concepto in enumerate(conceptos):
        ...
        try:
            cantidad=parse_decimal(cantidades[i]); costo=parse_decimal(costos[i])
        except Exception:
            flash('Revise las cantidades y los costos de los ítems.','danger'); return redirect(url_for('new_payment_request'))
        if costo < 0:
            flash('El costo unitario no puede ser negativo.','danger'); return redirect(url_for('new_payment_request'))
        unidad=unidades[i].strip() if i<len(unidades) else ''
        if not unidad:
            flash('Todo concepto debe tener unidad.','danger'); return redirect(url_for('new_payment_request'))
        despacho=despachos[i].strip() if i<len(despachos) else ''
        parsed.append((concepto,unidad,cantidad,costo,cantidad*costo,despacho))
    if not parsed:
        flash('Agregue al menos un concepto.','danger'); return redirect(url_for('new_payment_request'))
    total=sum(x[4] for x in parsed)
    usar_amort=request.form.get('usar_amortizacion')=='1'
    amort=parse_decimal(request.form.get('amortizacion','0')) if usar_amort else 0
    if amort < 0 or (total >= 0 and amort > total):
        flash('La amortización debe estar entre 0 y el total.','danger'); return redirect(url_for('new_payment_request'))
    ...
    if c.execute("SELECT 1 FROM tramites WHERE obra_id=? AND tipo='SP' AND LOWER(TRIM(numero))=LOWER(TRIM(?))",(obra['id'],numero)).fetchone():
        c.close(); flash(f'La solicitud N.° {numero} ya existe.','warning'); return redirect(url_for('new_payment_request'))
    ...
    for m in dict.fromkeys([m.strip() for m in modalidades if m.strip()]):
        c.execute('INSERT INTO sp_modalidades(tramite_id,modalidad) VALUES(?,?)',(tid,m))
    bancos=request.form.getlist('banco[]'); cuentas=request.form.getlist('cuenta_cci[]'); otros=request.form.getlist('banco_otro[]')
    for i,b in enumerate(bancos):
        b=b.strip(); cuenta=cuentas[i].strip() if i<len(cuentas) else ''
        if b=='Otro' and i<len(otros) and otros[i].strip(): b=otros[i].strip()
        if b: c.execute('INSERT INTO sp_cuentas(tramite_id,banco,cuenta_cci) VALUES(?,?,?)',(tid,b,cuenta))

    tipos=request.form.getlist('comprobante_tipo[]'); nums=request.form.getlist('comprobante_numero[]'); files=request.files.getlist('comprobante_archivo[]')
    maxn=max(len(tipos),len(nums),len(files))
    for i in range(maxn):
        tp=tipos[i].strip() if i<len(tipos) else ''; num=nums[i].strip() if i<len(nums) else ''; f=files[i] if i<len(files) else None
        if not tp and not num and not (f and f.filename): continue
        orig=guard=''
        if f and f.filename:
            if not f.filename.lower().endswith(('.pdf','.jpg','.jpeg','.png','.webp')): continue
            orig=f.filename; guard=secure_filename(f'SP_COMP_{tid}_{i+1}_{uuid.uuid4().hex[:8]}_{f.filename}'); f.save(os.path.join(UPLOADS,guard))
        c.execute('INSERT INTO sp_comprobantes(tramite_id,tipo,numero,nombre_original,nombre_archivo,orden) VALUES(?,?,?,?,?,?)',(tid,tp,num,orig,guard,i+1))

    orden=1
    for f in request.files.getlist('documentos'):
        if not f or not f.filename or not f.filename.lower().endswith('.pdf'): continue
        orig=f.filename; guard=secure_filename(f'SP_DOC_{tid}_{orden}_{uuid.uuid4().hex[:8]}_{f.filename}'); f.save(os.path.join(UPLOADS,guard))
        c.execute('INSERT INTO attachments(tramite_id,nombre_original,nombre_archivo,orden,fecha) VALUES(?,?,?,?,?)',(tid,orig,guard,orden,now)); orden+=1
    ...
    if u['role'] in ('Gerencia de Obra','Control y Planeamiento'):
        roles=[('Gerencia de Obra','Gerencia de Obra'),('Control y Planeamiento','Planeamiento')]
    else:
        roles=[('Gerencia de Obra','Gerencia de Obra')]
    for user_role,approval_role in roles:
        approver=c.execute('''SELECT u.id FROM users u JOIN usuario_obra uo ON uo.usuario_id=u.id WHERE u.role=? AND u.active=1 AND uo.obra_id=? AND uo.active=1 ORDER BY u.id LIMIT 1''',(user_role,obra['id'])).fetchone()
        if not approver:
            c.rollback(); c.close(); flash(f'La obra no tiene un usuario activo con el rol {user_role}.','danger'); return redirect(url_for('new_payment_request'))
        c.execute('INSERT INTO approvals(tramite_id,rol,usuario_id) VALUES(?,?,?)',(tid,approval_role,approver['id']))
    c.execute('INSERT INTO history(tramite_id,usuario_id,accion,fecha) VALUES(?,?,?,?)',(tid,u['id'],'Solicitud registrada: '+tracking,now))
    c.commit(); c.close(); return redirect(url_for('request_detail',tid=tid))
...
def edit_request(tid):
    ...
    if request.method == 'POST':
        ...
        if t['tipo'] == 'REQ' and request.form.get('numero_correlativo'):
            ...
            year_numero = numero_actual.split('-')[-1] if '-' in numero_actual else (fecha[:4] if fecha else str(datetime.now().year))
            try:
                numero = compose_form_number('REQ', request.form.get('numero_correlativo'), year_numero)
            except ValueError as exc:
                c.close()
                flash(str(exc), 'warning')
        ...
        if t['tipo'] == 'REQ':
            ...
            if duplicate:
                c.close()
                flash(f'El requerimiento N.° {numero} ya existe. Ingrese otro correlativo.', 'warning')
                return redirect(url_for('edit_request', tid=tid))
            ...
            c.execute(
                ...
                WHERE id=?
                """,
                (numero, fecha, 'F01A-LANR-' + (re.sub(r'\D', '', numero.split('-')[0]) or numero.split('-')[0]), tid)
            )
            ...
            for img in imagenes_previas:
                if img['item_key'] not in claves_actuales:
                    ...
                    c.execute('DELETE FROM item_imagenes WHERE id=?', (img['id'],))
            ...
            for (
                ...
                # Agregar nuevas imágenes seleccionadas para este ítem.
                for archivo in request.files.getlist(f'imagenes_{item_key}'):
                    if not archivo or not archivo.filename:
                        continue
                    ext = os.path.splitext(archivo.filename)[1].lower()
            ...
            for aid_txt in request.form.getlist('eliminar_adjuntos[]'):
                ...
                if os.path.isfile(ruta):
                    try:
                        ...
                        pass
                c.execute('DELETE FROM attachments WHERE id=? AND tramite_id=?', (aid, tid))

            # Si se carga un archivo nuevo, reemplaza al archivo externo anterior.
            nuevos_req = [f for f in request.files.getlist('documentos') if f and f.filename]
            if nuevos_req:
                ...
                if ext in ('.pdf', '.xls', '.xlsx', '.xlsm'):
                    ...
                    for anterior in anteriores:
                        ruta_anterior = os.path.join(UPLOADS, anterior['nombre_archivo'])
        ...
        # =====================================================
            ...
            subtipo=request.form.get('subtipo','').strip(); beneficiario=request.form.get('beneficiario','').strip()
            valid_types=('Proveedor persona natural','Proveedor persona jurídica','Cuarta categoría / Recibo por Honorarios','Planillas')
            if subtipo not in valid_types or not beneficiario:
                c.close(); flash('Tipo de solicitud y beneficiario son obligatorios.','danger'); return redirect(url_for('edit_request',tid=tid))
            moneda=request.form.get('moneda','PEN'); moneda=moneda if moneda in ('PEN','USD') else 'PEN'
            conceptos=request.form.getlist('concepto[]'); unidades=request.form.getlist('unidad_sp[]'); cantidades=request.form.getlist('cantidad_sp[]'); costos=request.form.getlist('costo[]'); despachos=request.form.getlist('nro_despacho[]')
            sp_items=[]
            for i,concepto in enumerate(conceptos):
                concepto=concepto.strip()
                if not concepto: continue
                unidad=unidades[i].strip() if i<len(unidades) else ''; cantidad_txt=cantidades[i].strip() if i<len(cantidades) else ''; costo_txt=costos[i].strip() if i<len(costos) else ''; despacho=despachos[i].strip() if i<len(despachos) else ''
                if not unidad or cantidad_txt=='' or costo_txt=='':
                    c.close(); flash(f'Complete unidad, cantidad y costo del ítem {i+1}.','danger'); return redirect(url_for('edit_request',tid=tid))
                cantidad=parse_decimal(cantidad_txt); costo=parse_decimal(costo_txt)
                if costo<0:
                    c.close(); flash(f'El costo del ítem {i+1} no puede ser negativo.','danger'); return redirect(url_for('edit_request',tid=tid))
                sp_items.append((concepto,unidad,cantidad,costo,cantidad*costo,despacho))
            if not sp_items:
                c.close(); flash('La solicitud debe conservar al menos un ítem.','danger'); return redirect(url_for('edit_request',tid=tid))
            total=sum(x[4] for x in sp_items); usar_amort=request.form.get('usar_amortizacion')=='1'; amort=parse_decimal(request.form.get('amortizacion','0')) if usar_amort else 0
            if amort<0 or (total>=0 and amort>total):
                c.close(); flash('La amortización no es válida.','danger'); return redirect(url_for('edit_request',tid=tid))
            abono=amort if usar_amort else total; saldo=total-amort if usar_amort else 0
            celular=re.sub(r'\D','',request.form.get('celular',''))[:15]
            ...
            c.execute('DELETE FROM items WHERE tramite_id=?',(tid,))
            for nro,x in enumerate(sp_items,1): c.execute('INSERT INTO items(tramite_id,nro,descripcion,unidad,cantidad,costo,monto,nro_despacho) VALUES(?,?,?,?,?,?,?,?)',(tid,nro,*x))
            c.execute('DELETE FROM sp_modalidades WHERE tramite_id=?',(tid,)); mods=request.form.getlist('modalidad_pago[]'); other=request.form.get('modalidad_otro','').strip()
            if 'Otro' in mods and other: mods=[m for m in mods if m!='Otro']+[other]
            for m in dict.fromkeys([m.strip() for m in mods if m.strip()]): c.execute('INSERT INTO sp_modalidades(tramite_id,modalidad) VALUES(?,?)',(tid,m))
            c.execute('DELETE FROM sp_cuentas WHERE tramite_id=?',(tid,)); bancos=request.form.getlist('banco[]'); cuentas=request.form.getlist('cuenta_cci[]'); otros=request.form.getlist('banco_otro[]')
            for i,b in enumerate(bancos):
                b=b.strip(); cuenta=cuentas[i].strip() if i<len(cuentas) else ''
                if b=='Otro' and i<len(otros) and otros[i].strip(): b=otros[i].strip()
                if b: c.execute('INSERT INTO sp_cuentas(tramite_id,banco,cuenta_cci) VALUES(?,?,?)',(tid,b,cuenta))
            ...
            for cid_txt in request.form.getlist('eliminar_comprobantes[]'):
                ...
                row=c.execute('SELECT * FROM sp_comprobantes WHERE id=? AND tramite_id=?',(cid,tid)).fetchone()
                if row:
                    if row['nombre_archivo']:
                        try: os.remove(os.path.join(UPLOADS,row['nombre_archivo']))
                        except OSError: pass
                    c.execute('DELETE FROM sp_comprobantes WHERE id=?',(cid,))
            tipos=request.form.getlist('comprobante_tipo[]'); nums=request.form.getlist('comprobante_numero[]'); files=request.files.getlist('comprobante_archivo[]')
            start=c.execute('SELECT COALESCE(MAX(orden),0) n FROM sp_comprobantes WHERE tramite_id=?',(tid,)).fetchone()['n']
            for i in range(max(len(tipos),len(nums),len(files))):
                ...
                if f and f.filename:
                    if not f.filename.lower().endswith(('.pdf','.jpg','.jpeg','.png','.webp')): continue
                    orig=f.filename; guard=secure_filename(f'SP_COMP_{tid}_{uuid.uuid4().hex[:8]}_{f.filename}'); f.save(os.path.join(UPLOADS,guard))
                start+=1; c.execute('INSERT INTO sp_comprobantes(tramite_id,tipo,numero,nombre_original,nombre_archivo,orden) VALUES(?,?,?,?,?,?)',(tid,tp,num,orig,guard,start))
            for aid_txt in request.form.getlist('eliminar_adjuntos[]'):
                try: aid=int(aid_txt)
                except: continue
                a=c.execute('SELECT * FROM attachments WHERE id=? AND tramite_id=?',(aid,tid)).fetchone()
                if a:
                    try: os.remove(os.path.join(UPLOADS,a['nombre_archivo']))
                    except OSError: pass
                    c.execute('DELETE FROM attachments WHERE id=?',(aid,))
            order=c.execute('SELECT COALESCE(MAX(orden),0) n FROM attachments WHERE tramite_id=?',(tid,)).fetchone()['n']
            for f in request.files.getlist('documentos'):
                if not f or not f.filename or not f.filename.lower().endswith('.pdf'): continue
                order+=1; guard=secure_filename(f'SP_DOC_{tid}_{order}_{uuid.uuid4().hex[:8]}_{f.filename}'); f.save(os.path.join(UPLOADS,guard)); c.execute('INSERT INTO attachments(tramite_id,nombre_original,nombre_archivo,orden,fecha) VALUES(?,?,?,?,?)',(tid,f.filename,guard,order,datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
    ...
    sp_modalidades = c.execute('SELECT * FROM sp_modalidades WHERE tramite_id=? ORDER BY id',(tid,)).fetchall() if t['tipo']=='SP' else []
    sp_cuentas = c.execute('SELECT * FROM sp_cuentas WHERE tramite_id=? ORDER BY id',(tid,)).fetchall() if t['tipo']=='SP' else []
    sp_comprobantes = c.execute('SELECT * FROM sp_comprobantes WHERE tramite_id=? ORDER BY orden,id',(tid,)).fetchall() if t['tipo']=='SP' else []
    c.close()
...
def requests():
    ...
    ts = c.execute(
        ...
        JOIN users creador ON creador.id=t.creador
        WHERE t.obra_id=?
          AND (t.estado!='Pendiente de mi revisión' OR t.creador=?)
        ORDER BY t.id DESC
        ''',
        (obra['id'], u['id'])
    ).fetchall()
...
def request_tracking(tid):
    ...
    if t['estado'] == 'Pendiente de mi revisión' and t['creador'] != u['id']:
        c.close()
        abort(404)
...
def request_detail(tid):
    ...
    # Un trámite que todavía está en revisión del creador es privado.
    # Nadie más puede verlo hasta que el creador registre su V°B°.
    if (
        t['estado'] == 'Pendiente de mi revisión'
        and t['creador'] != u['id']
    ):
    ...
    sp_cuentas = c.execute('SELECT * FROM sp_cuentas WHERE tramite_id=? ORDER BY id',(tid,)).fetchall() if t['tipo']=='SP' else []
    sp_comprobantes = c.execute('SELECT * FROM sp_comprobantes WHERE tramite_id=? ORDER BY orden,id',(tid,)).fetchall() if t['tipo']=='SP' else []
    sp_pagos = c.execute('''SELECT p.*,u.full_name pagador_nombre FROM sp_pagos_multiples p JOIN users u ON u.id=p.pagado_por WHERE p.tramite_id=? ORDER BY p.id''',(tid,)).fetchall() if t['tipo']=='SP' else []
...
def view_sp_comprobante(cid):
    c=db(); row=c.execute('''SELECT sc.* FROM sp_comprobantes sc JOIN tramites t ON t.id=sc.tramite_id WHERE sc.id=? AND t.obra_id=?''',(cid,active_project()['id'])).fetchone(); c.close()
    if not row or not row['nombre_archivo']: abort(404)
...
def view_sp_multiple_payment(pid):
    c=db(); row=c.execute('''SELECT p.* FROM sp_pagos_multiples p JOIN tramites t ON t.id=p.tramite_id WHERE p.id=? AND t.obra_id=?''',(pid,active_project()['id'])).fetchone(); c.close()
...
def assign_sp_payment(tid):
    u=user()
    if u['role']!='Administración': abort(403)
    asignado=request.form.get('asignado_pago','').strip()
    if asignado not in ('Tesorería','Gerencia General'):
        flash('Seleccione quién realizará el pago.','danger'); return redirect(url_for('request_detail',tid=tid))
    c=db(); t=c.execute("SELECT * FROM tramites WHERE id=? AND tipo='SP'",(tid,)).fetchone()
    if not t: c.close(); abort(404)
    if t['estado']!='Pendiente de Administración':
        c.close(); flash('Esta solicitud no está pendiente de Administración.','warning'); return redirect(url_for('request_detail',tid=tid))
    now=datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    c.execute('INSERT OR IGNORE INTO gestion_sp(tramite_id) VALUES(?)',(tid,))
    c.execute('UPDATE gestion_sp SET asignado_pago=?,asignado_por=?,fecha_asignacion=? WHERE tramite_id=?',(asignado,u['id'],now,tid))
    ...
    c.execute('INSERT INTO history(tramite_id,usuario_id,accion,fecha) VALUES(?,?,?,?)',(tid,u['id'],f'Administración revisó y asignó el pago a {asignado}',now))
    destino_role='Tesorería' if asignado=='Tesorería' else 'Gerencia General'
    destinos=c.execute('''SELECT u.id FROM users u JOIN usuario_obra uo ON uo.usuario_id=u.id WHERE u.role=? AND u.active=1 AND uo.obra_id=? AND uo.active=1''',(destino_role,t['obra_id'])).fetchall()
    c.commit(); c.close()
    for d in destinos: notify(d['id'],tid,'Solicitud asignada para pago',f'La solicitud {t["tracking"]} fue asignada a {asignado}.','accion')
    flash('Solicitud revisada y responsable de pago asignado.','success'); return redirect(url_for('request_detail',tid=tid))
...
def register_sp_payment(tid):
    u=user(); c=db()
    t=c.execute("SELECT * FROM tramites WHERE id=? AND tipo='SP'",(tid,)).fetchone()
    g=c.execute('SELECT * FROM gestion_sp WHERE tramite_id=?',(tid,)).fetchone()
    if not t or not g: c.close(); abort(404)
    expected='Tesorería' if g['asignado_pago']=='Tesorería' else 'Gerencia General'
    if u['role']!=expected: c.close(); abort(403)
    if t['estado'] not in ('Asignada a Tesorería','Asignada a Gerencia General','Pago parcial'):
        c.close(); flash('La solicitud no está habilitada para registrar pagos.','warning'); return redirect(url_for('request_detail',tid=tid))
    medio=request.form.get('medio_pago','').strip(); banco=request.form.get('banco_pago','').strip(); nro=request.form.get('nro_operacion','').strip(); fecha=request.form.get('fecha_pago','').strip()
    try: monto=parse_decimal(request.form.get('monto_pagado','0'))
    except: monto=0
    ...
    if not medio or not fecha or monto<=0 or not evidencia or not evidencia.filename:
        c.close(); flash('Medio, fecha, monto y evidencia de pago son obligatorios.','danger'); return redirect(url_for('request_detail',tid=tid))
    pagado=float(c.execute('SELECT COALESCE(SUM(monto),0) v FROM sp_pagos_multiples WHERE tramite_id=?',(tid,)).fetchone()['v'])
    objetivo=float(t['abono'] or 0); restante=max(0,objetivo-pagado)
    if monto>restante+0.01:
        c.close(); flash(f'El monto supera el saldo por pagar ({restante:,.2f}).','danger'); return redirect(url_for('request_detail',tid=tid))
    now=datetime.now().strftime('%Y-%m-%d %H:%M:%S'); orig=evidencia.filename; guard=secure_filename(f'SP_PAGO_{tid}_{uuid.uuid4().hex[:10]}_{orig}'); evidencia.save(os.path.join(UPLOADS,guard))
    c.execute('''INSERT INTO sp_pagos_multiples(tramite_id,pagado_por,medio_pago,banco_pago,nro_operacion,monto,fecha_pago,nombre_original,nombre_archivo,creado) VALUES(?,?,?,?,?,?,?,?,?,?)''',(tid,u['id'],medio,banco,nro,monto,fecha,orig,guard,now))
    nuevo=pagado+monto
    final=nuevo+0.01>=objetivo
    estado='Pagada pendiente conformidad GG' if final else 'Pago parcial'
    c.execute('UPDATE tramites SET estado=? WHERE id=?',(estado,tid))
    c.execute('UPDATE gestion_sp SET pagado_por=?,medio_pago=?,banco_pago=?,nro_operacion=?,monto_pagado=?,fecha_pago=?,nombre_original_pago=?,nombre_archivo_pago=? WHERE tramite_id=?',(u['id'],medio,banco,nro,nuevo,fecha,orig,guard,tid))
    c.execute('INSERT INTO history(tramite_id,usuario_id,accion,fecha) VALUES(?,?,?,?)',(tid,u['id'],f'Pago registrado: {monto:,.2f}. Acumulado: {nuevo:,.2f}',now))
    c.commit(); c.close(); flash('Pago registrado correctamente.' if final else 'Pago parcial registrado. La solicitud mantiene saldo pendiente.','success'); return redirect(url_for('request_detail',tid=tid))
...
def edit_unit(unit_id):
    ...
    c = db()
    target = c.execute('SELECT * FROM unidades_catalogo WHERE id=?', (unit_id,)).fetchone()
    if not target:
        c.close()
        abort(404)
...
def toggle_unit_status(unit_id):
    ...
    c = db()
    row = c.execute('SELECT * FROM unidades_catalogo WHERE id=?', (unit_id,)).fetchone()
    if not row:
        c.close()
        abort(404)
...
def logistics_quotations(tid):
    u=user(); obra=active_project(); c=db()
    t=c.execute("SELECT * FROM tramites WHERE id=? AND obra_id=?",(tid,obra['id'])).fetchone()
    if not t or t['tipo']!='REQ': c.close(); abort(404)
    if u['role'] not in ('Logística','Administración','Sistemas'): c.close(); abort(403)
    items=c.execute("SELECT * FROM items WHERE tramite_id=? ORDER BY seccion,nro",(tid,)).fetchall()
    proveedores=c.execute("SELECT * FROM proveedores WHERE active=1 ORDER BY nombre").fetchall()
    ...
    qfiles=c.execute("""SELECT f.* FROM cotizacion_archivos f JOIN cotizaciones q ON q.id=f.cotizacion_id WHERE q.tramite_id=? ORDER BY f.id""",(tid,)).fetchall()
    ...
    pagos=c.execute("SELECT * FROM solicitudes_tesoreria WHERE tramite_id=? AND origen='REQ-Cotizacion' ORDER BY id DESC",(tid,)).fetchall()
...
def provider_create():
    u=user()
    if u['role'] not in ('Logística','Administración','Sistemas'): abort(403)
    tid=int(request.form.get('tid') or 0); nombre=request.form.get('nombre','').strip(); documento=request.form.get('documento','').strip()
    if not nombre: flash('Ingrese el nombre o razón social del proveedor.','danger'); return redirect(url_for('logistics_quotations',tid=tid))
    c=db(); now=datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    existe=c.execute("SELECT id FROM proveedores WHERE LOWER(nombre)=LOWER(?)",(nombre,)).fetchone()
    if existe: flash('Ese proveedor ya está registrado.','warning')
    else:
        c.execute("INSERT INTO proveedores(nombre,documento,creado,actualizado) VALUES(?,?,?,?)",(nombre,documento or None,now,now)); c.commit(); flash('Proveedor registrado.','success')
    c.close(); return redirect(url_for('logistics_quotations',tid=tid))

@app.route('/logistics/<int:tid>/quotations/create', methods=['POST'])
@login_required
def quotation_create(tid):
    u=user()
    if u['role']!='Logística': abort(403)
    c=db(); t=c.execute("SELECT * FROM tramites WHERE id=?",(tid,)).fetchone()
    if not t or t['tipo']!='REQ' or t['estado'] not in ('Recibido por Logística','En gestión de compra','Pendiente de cotización','Cotizaciones en gestión'): c.close(); abort(400)
    proveedor_id=request.form.get('proveedor_id','').strip(); tipo=request.form.get('tipo_sustento','Cotización').strip(); fecha=request.form.get('fecha','').strip() or datetime.now().strftime('%Y-%m-%d'); obs=request.form.get('observacion','').strip()
    if tipo not in ('Cotización','Proveedor habitual'): c.close(); abort(400)
    if not proveedor_id: c.close(); flash('Seleccione un proveedor.','danger'); return redirect(url_for('logistics_quotations',tid=tid))
    item_ids=request.form.getlist('item_id[]'); cantidades=request.form.getlist('cantidad[]'); precios=request.form.getlist('precio_unitario[]')
    filas=[]
    for idx,item_id in enumerate(item_ids):
        qty=parse_decimal(cantidades[idx] if idx<len(cantidades) else ''); price=parse_decimal(precios[idx] if idx<len(precios) else '')
        if qty>0 or price>0:
            if qty<=0 or price<=0: c.close(); flash('Cada producto seleccionado debe tener cantidad y precio unitario mayores que 0.','danger'); return redirect(url_for('logistics_quotations',tid=tid))
            filas.append((int(item_id),qty,price))
    if not filas: c.close(); flash('Seleccione al menos un producto con cantidad y precio.','danger'); return redirect(url_for('logistics_quotations',tid=tid))
    archivos=[f for f in request.files.getlist('archivos') if f and f.filename]
    if tipo=='Cotización' and not archivos: c.close(); flash('Adjunte el PDF o imagen de la cotización.','danger'); return redirect(url_for('logistics_quotations',tid=tid))
    permitidos=('.pdf','.png','.jpg','.jpeg','.webp')
    if any(not f.filename.lower().endswith(permitidos) for f in archivos): c.close(); flash('Los archivos deben ser PDF o imagen.','danger'); return redirect(url_for('logistics_quotations',tid=tid))
    now=datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    cur=c.execute("INSERT INTO cotizaciones(tramite_id,proveedor_id,tipo_sustento,fecha,estado,observacion,creado_por,creado) VALUES(?,?,?,?,?,?,?,?)",(tid,int(proveedor_id),tipo,fecha,'Borrador',obs,u['id'],now)); qid=cur.lastrowid
    for item_id,qty,price in filas: c.execute("INSERT INTO cotizacion_items(cotizacion_id,item_id,cantidad,precio_unitario) VALUES(?,?,?,?)",(qid,item_id,qty,price))
    for idx,f in enumerate(archivos,1):
        original=f.filename; guardado=secure_filename(f'COT_{tid}_{qid}_{idx}_{datetime.now().strftime("%Y%m%d%H%M%S%f")}_{f.filename}'); f.save(os.path.join(UPLOADS,guardado)); c.execute("INSERT INTO cotizacion_archivos(cotizacion_id,nombre_original,nombre_archivo,creado) VALUES(?,?,?,?)",(qid,original,guardado,now))
    c.execute("UPDATE tramites SET estado='Cotizaciones en gestión' WHERE id=?",(tid,)); c.execute("INSERT INTO history(tramite_id,usuario_id,accion,fecha) VALUES(?,?,?,?)",(tid,u['id'],'Cotización guardada en borrador',now)); c.commit(); c.close(); flash('Cotización guardada como borrador.','success'); return redirect(url_for('logistics_quotations',tid=tid))
...
def quotation_send(qid):
    u=user()
    if u['role']!='Logística': abort(403)
    c=db(); q=c.execute("SELECT q.*,t.tracking FROM cotizaciones q JOIN tramites t ON t.id=q.tramite_id WHERE q.id=?",(qid,)).fetchone()
    if not q or q['estado']!='Borrador': c.close(); abort(400)
    now=datetime.now().strftime('%Y-%m-%d %H:%M:%S'); c.execute("UPDATE cotizaciones SET estado='Enviada a Sara',enviado_fecha=? WHERE id=?",(now,qid)); c.execute("INSERT INTO history(tramite_id,usuario_id,accion,fecha) VALUES(?,?,?,?)",(q['tramite_id'],u['id'],f'Cotización #{qid} enviada a Administración',now)); sara=c.execute("SELECT id FROM users WHERE role='Administración' AND active=1 ORDER BY id LIMIT 1").fetchone(); c.commit(); c.close()
    if sara: notify(sara['id'],q['tramite_id'],'Cotización pendiente de revisión',f"Logística envió una cotización del requerimiento {q['tracking']} para revisión.",'accion')
    flash('Cotización enviada a Sara.','success'); return redirect(url_for('logistics_quotations',tid=q['tramite_id']))

@app.route('/quotations/files/<int:fid>')
@login_required
def quotation_file(fid):
    c=db(); f=c.execute("SELECT * FROM cotizacion_archivos WHERE id=?",(fid,)).fetchone(); c.close()
    if not f: abort(404)
...
def quotation_authorize(tid):
    u=user()
    if u['role']!='Administración': abort(403)
    c=db(); selections=request.form.getlist('selection[]'); grouped=[]
    # each value qid:itemid, with qty_{qid}_{itemid}
    current_auth=dict((r['item_id'],float(r['qty'] or 0)) for r in c.execute("SELECT ai.item_id,SUM(ai.cantidad) qty FROM autorizacion_items ai JOIN autorizaciones_compra a ON a.id=ai.autorizacion_id WHERE a.tramite_id=? AND a.estado='Autorizada' GROUP BY ai.item_id",(tid,)).fetchall())
    for token in selections:
        try: qid,item_id=map(int,token.split(':'))
        except: continue
        ...
        if not row or row['estado']!='Enviada a Sara' or qty<=0 or qty>float(row['cantidad'] or 0): continue
        pendiente=max(0,float(row['comprar'] or 0)-current_auth.get(item_id,0))
        if qty>pendiente+0.0001: c.close(); flash('Una cantidad autorizada supera lo pendiente por comprar.','danger'); return redirect(url_for('logistics_quotations',tid=tid))
        grouped.append((qid,item_id,row['proveedor_id'],qty,float(row['precio_unitario'])))
        current_auth[item_id]=current_auth.get(item_id,0)+qty
    if not grouped: c.close(); flash('Seleccione al menos una cantidad para autorizar.','danger'); return redirect(url_for('logistics_quotations',tid=tid))
    now=datetime.now().strftime('%Y-%m-%d %H:%M:%S'); cur=c.execute("INSERT INTO autorizaciones_compra(tramite_id,autorizado_por,estado,fecha) VALUES(?,?,'Autorizada',?)",(tid,u['id'],now)); aid=cur.lastrowid
    totals={}
    for qid,item_id,pid,qty,price in grouped:
        subtotal=qty*price; c.execute("INSERT INTO autorizacion_items(autorizacion_id,cotizacion_id,item_id,proveedor_id,cantidad,precio_unitario,subtotal) VALUES(?,?,?,?,?,?,?)",(aid,qid,item_id,pid,qty,price,subtotal)); totals[pid]=totals.get(pid,0)+subtotal
    yoana=c.execute("SELECT id FROM users WHERE role='Tesorería' AND active=1 ORDER BY id LIMIT 1").fetchone()
    for pid,total in totals.items():
        prov=c.execute("SELECT nombre FROM proveedores WHERE id=?",(pid,)).fetchone(); c.execute("INSERT INTO solicitudes_tesoreria(tramite_id,origen,solicitado_por,motivo,monto,estado,fecha_solicitud,autorizacion_id,proveedor_id,proveedor_nombre) VALUES(?, 'REQ-Cotizacion', ?, ?, ?, 'Pendiente', ?, ?, ?, ?)",(tid,u['id'],f'Compra autorizada - {prov["nombre"]}',total,now,aid,pid,prov['nombre']))
    c.execute("UPDATE tramites SET estado='En gestión de compra' WHERE id=?",(tid,)); c.execute("INSERT INTO history(tramite_id,usuario_id,accion,fecha) VALUES(?,?,?,?)",(tid,u['id'],f'Compra autorizada parcialmente/completa - autorización #{aid}',now)); c.commit(); c.close()
    if yoana: notify(yoana['id'],tid,'Pago de compra autorizado',f'Administración autorizó una compra del requerimiento. Tienes {len(totals)} pago(s) por atender.','accion')
    flash('Compra autorizada. Los pagos pendientes ya aparecen a Tesorería.','success'); return redirect(url_for('logistics_quotations',tid=tid))
...
def authorization_cancel(aid):
    u=user()
    if u['role']!='Administración': abort(403)
    motivo=request.form.get('motivo','').strip(); c=db(); a=c.execute("SELECT * FROM autorizaciones_compra WHERE id=?",(aid,)).fetchone()
    if not a or a['estado']!='Autorizada': c.close(); abort(400)
    paid=c.execute("SELECT COUNT(*) n FROM solicitudes_tesoreria WHERE autorizacion_id=? AND estado='Atendido'",(aid,)).fetchone()['n']
    if paid: c.close(); flash('No puede anular: Yoana ya atendió al menos un pago de esta autorización.','danger'); return redirect(url_for('logistics_quotations',tid=a['tramite_id']))
    if not motivo: c.close(); flash('Indique el motivo de anulación.','danger'); return redirect(url_for('logistics_quotations',tid=a['tramite_id']))
    now=datetime.now().strftime('%Y-%m-%d %H:%M:%S'); c.execute("UPDATE autorizaciones_compra SET estado='Anulada',motivo_anulacion=?,anulada_fecha=? WHERE id=?",(motivo,now,aid)); c.execute("UPDATE solicitudes_tesoreria SET estado='Anulado',fecha_atencion=? WHERE autorizacion_id=? AND estado='Pendiente'",(now,aid)); c.execute("INSERT INTO history(tramite_id,usuario_id,accion,fecha) VALUES(?,?,?,?)",(a['tramite_id'],u['id'],f'Autorización de compra #{aid} anulada: {motivo}',now)); c.commit(); c.close(); flash('Autorización anulada.','success'); return redirect(url_for('logistics_quotations',tid=a['tramite_id']))
...
def logistics_purchase(tid):
    ...
    fecha = request.form.get('fecha_cotizacion', '').strip() or datetime.now().strftime('%Y-%m-%d')
    cotizacion = request.files.get('cotizacion')

    if not proveedor_o_ruc:
        c.close(); flash('Ingrese el proveedor o el RUC de la compra.', 'danger'); return redirect(url_for('request_detail', tid=tid))

    solo_digitos = re.sub(r'\D', '', proveedor_o_ruc)
    if proveedor_o_ruc.isdigit():
        if len(solo_digitos) != 11:
            c.close(); flash('Si ingresas un RUC, debe tener 11 dígitos.', 'danger'); return redirect(url_for('request_detail', tid=tid))
        ruc = solo_digitos
        proveedor = ruc
    else:
        ...
        ruc = ''
    if monto <= 0:
        c.close(); flash('Ingrese un monto cotizado mayor que 0.', 'danger'); return redirect(url_for('request_detail', tid=tid))
    if cotizacion and cotizacion.filename and not cotizacion.filename.lower().endswith(('.pdf','.png','.jpg','.jpeg','.webp')):
        c.close(); flash('La cotización debe ser PDF o imagen.', 'danger'); return redirect(url_for('request_detail', tid=tid))

    now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    c.execute('INSERT OR IGNORE INTO gestion_logistica(tramite_id) VALUES(?)', (tid,))
...
def logistics_receipts(tid):
    ...
    c=db()
    t=c.execute('SELECT * FROM tramites WHERE id=?',(tid,)).fetchone()
    g=c.execute('SELECT * FROM gestion_logistica WHERE tramite_id=?',(tid,)).fetchone()
    if not t or not g or g['estado_pago'] not in ('Pagado por Logística','Pagado por Tesorería'):
        c.close(); flash('Primero debe quedar gestionado el pago.', 'danger'); return redirect(url_for('request_detail',tid=tid))

    fechas=request.form.getlist('fecha_compra[]'); tipos=request.form.getlist('tipo_comprobante[]')
    numeros=request.form.getlist('nro_comprobante[]'); montos=request.form.getlist('monto[]')
    archivos=request.files.getlist('comprobante[]'); filas=[]
    n=max(len(fechas),len(tipos),len(numeros),len(montos),len(archivos))
    permitidos=('.pdf','.png','.jpg','.jpeg','.webp','.xml')
    for i in range(n):
        fecha=fechas[i].strip() if i<len(fechas) else ''; tipo=tipos[i].strip() if i<len(tipos) else ''
        numero=numeros[i].strip() if i<len(numeros) else ''; monto=parse_decimal(montos[i] if i<len(montos) else '')
        archivo=archivos[i] if i<len(archivos) else None
        if not any((fecha,tipo,numero,monto, archivo and archivo.filename)): continue
        if not fecha or tipo not in ('Factura','Boleta','Otro') or not numero or monto<=0 or not archivo or not archivo.filename:
            c.close(); flash('Complete la fecha, tipo, número, monto y evidencia de cada comprobante.', 'danger'); return redirect(url_for('request_detail',tid=tid))
        if not archivo.filename.lower().endswith(permitidos):
            c.close(); flash('La evidencia debe ser PDF, imagen o XML.', 'danger'); return redirect(url_for('request_detail',tid=tid))
        filas.append((fecha,tipo,numero,monto,archivo))
    if not filas:
        c.close(); flash('Registra al menos un comprobante definitivo.', 'danger'); return redirect(url_for('request_detail',tid=tid))

    now=datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    total=0; nums=[]
    for idx,(fecha,tipo,numero,monto,archivo) in enumerate(filas,1):
        original=archivo.filename
        guardado=secure_filename(f'COMPRA_{tid}_{idx}_{datetime.now().strftime("%Y%m%d%H%M%S%f")}_{archivo.filename}')
        archivo.save(os.path.join(UPLOADS,guardado))
        ...
        total+=monto; nums.append(numero)
    ...
    create_regularization(c,tid,u['id'],'Regularización de compra','Detallar los materiales comprados, cantidades y comprobante que sustenta cada adquisición.')
    c.execute('INSERT INTO history(tramite_id,usuario_id,accion,fecha) VALUES(?,?,?,?)',(tid,u['id'],f'Comprobante(s) definitivo(s) registrados por Logística - S/ {total:,.2f}',now))
    c.commit(); c.close()
...
def treasury_dashboard():
    ...
    pagos_compra = c.execute(q+" AND s.origen IN ('REQ-Cotizacion','REQ-Compra') ORDER BY CASE WHEN s.estado='Pendiente' THEN 0 ELSE 1 END,s.id DESC", (obra['id'],)).fetchall()
    reembolsos = c.execute(q+" AND s.estado='Pendiente' AND s.origen='REQ-Reembolso' ORDER BY s.id DESC", (obra['id'],)).fetchall()
    sp_pagos = c.execute('''
    ...
    ''',(obra['id'],)).fetchall()
    rr_pendientes = c.execute("SELECT COUNT(*) n FROM reembolsos_rendiciones WHERE obra_id=? AND estado='Pendiente de Tesorería'",(obra['id'],)).fetchone()['n']
    c.close()
...
def treasury_pay(sid):
    ...
    if solicitud['origen'] in ('REQ-Compra','REQ-Cotizacion'):
        pendientes = c.execute("SELECT COUNT(*) n FROM solicitudes_tesoreria WHERE tramite_id=? AND origen='REQ-Cotizacion' AND estado='Pendiente'",(tid,)).fetchone()['n']
        if pendientes == 0:
            c.execute("INSERT OR IGNORE INTO gestion_logistica(tramite_id) VALUES(?)",(tid,))
            c.execute("UPDATE gestion_logistica SET estado_pago='Pagado por Tesorería' WHERE tramite_id=?", (tid,))
...
def logistics_dispatch(tid):
    ...
    if guia_opcion == 'adjuntar':
        if not guia_numero or not guia_fecha or not archivos_guia:
            flash('Para adjuntar la guía, complete el número, la fecha y el archivo.', 'danger')
            return redirect(url_for('request_detail', tid=tid))
    else:
...
# =========================================================
REEMBURSEMENT_ROLES=('Gerencia de Obra','Control y Planeamiento','Administración','Logística','Tesorería','Contabilidad','Sistemas')

@app.route('/reimbursements')
@login_required
def reimbursements():
    u=user(); obra=active_project(); c=db()
    rows=c.execute('''SELECT r.*,u.full_name solicitante_nombre FROM reembolsos_rendiciones r JOIN users u ON u.id=r.solicitante_id WHERE r.obra_id=? ORDER BY r.id DESC''',(obra['id'],)).fetchall()
    files=c.execute('''SELECT a.* FROM reembolso_adjuntos a JOIN reembolsos_rendiciones r ON r.id=a.reembolso_id WHERE r.obra_id=? ORDER BY a.id''',(obra['id'],)).fetchall(); c.close()
...
def new_reimbursement():
    u=user(); obra=active_project()
    if u['role'] not in REEMBURSEMENT_ROLES: abort(403)
    if request.method=='GET': return render_template('new_reimbursement.html',user=u,obra=obra)
    tipo=request.form.get('tipo',''); fecha=request.form.get('fecha',''); concepto=request.form.get('concepto','').strip(); moneda=request.form.get('moneda','PEN'); obs=request.form.get('observaciones','').strip()
    try: monto=parse_decimal(request.form.get('monto','0'))
    except: monto=0
    if tipo not in ('Reembolso','Rendición') or not fecha or not concepto or monto<=0:
        flash('Complete tipo, fecha, concepto y monto.','danger'); return redirect(url_for('new_reimbursement'))
    c=db(); seq=c.execute('SELECT COALESCE(MAX(id),0)+1 n FROM reembolsos_rendiciones WHERE obra_id=?',(obra['id'],)).fetchone()['n']; numero=f'RR-{fecha[:4]}-{seq:03d}'; now=datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    cur=c.execute('''INSERT INTO reembolsos_rendiciones(obra_id,tipo,numero,fecha,solicitante_id,concepto,monto,moneda,observaciones,estado,creado) VALUES(?,?,?,?,?,?,?,?,?,'Pendiente de Administración',?)''',(obra['id'],tipo,numero,fecha,u['id'],concepto,monto,moneda,obs,now)); rid=cur.lastrowid
    for f in request.files.getlist('documentos'):
        if f and f.filename:
            guard=secure_filename(f'RR_{rid}_{uuid.uuid4().hex[:8]}_{f.filename}'); f.save(os.path.join(UPLOADS,guard)); c.execute('INSERT INTO reembolso_adjuntos(reembolso_id,nombre_original,nombre_archivo,creado) VALUES(?,?,?,?)',(rid,f.filename,guard,now))
    c.commit(); c.close(); flash(f'{tipo} registrado.','success'); return redirect(url_for('reimbursements'))
...
def authorize_reimbursement(rid):
    u=user()
    if u['role']!='Administración': abort(403)
    c=db(); r=c.execute('SELECT * FROM reembolsos_rendiciones WHERE id=? AND obra_id=?',(rid,active_project()['id'])).fetchone()
    if not r or r['estado']!='Pendiente de Administración': c.close(); abort(400)
    now=datetime.now().strftime('%Y-%m-%d %H:%M:%S'); c.execute("UPDATE reembolsos_rendiciones SET estado='Pendiente de Tesorería',autorizado_por=?,fecha_autorizacion=? WHERE id=?",(u['id'],now,rid)); c.commit(); c.close(); flash('Autorizado y enviado a Tesorería.','success'); return redirect(url_for('reimbursements'))

@app.route('/reimbursements/<int:rid>/attend', methods=['POST'])
@login_required
def attend_reimbursement(rid):
    u=user()
    if u['role']!='Tesorería': abort(403)
    evidencia=request.files.get('evidencia')
    if not evidencia or not evidencia.filename: flash('Adjunte evidencia de atención.','danger'); return redirect(url_for('reimbursements'))
    c=db(); r=c.execute('SELECT * FROM reembolsos_rendiciones WHERE id=? AND obra_id=?',(rid,active_project()['id'])).fetchone()
    if not r or r['estado']!='Pendiente de Tesorería': c.close(); abort(400)
    now=datetime.now().strftime('%Y-%m-%d %H:%M:%S'); guard=secure_filename(f'RR_PAGO_{rid}_{uuid.uuid4().hex[:8]}_{evidencia.filename}'); evidencia.save(os.path.join(UPLOADS,guard)); c.execute('INSERT INTO reembolso_adjuntos(reembolso_id,tipo,nombre_original,nombre_archivo,creado) VALUES(?,?,?,?,?)',(rid,'Evidencia de atención',evidencia.filename,guard,now)); c.execute("UPDATE reembolsos_rendiciones SET estado='Atendido',atendido_por=?,fecha_atencion=? WHERE id=?",(u['id'],now,rid)); c.commit(); c.close(); flash('Atención registrada.','success'); return redirect(url_for('reimbursements'))

@app.route('/reimbursements/files/<int:fid>')
@login_required
def reimbursement_file(fid):
    c=db(); f=c.execute('''SELECT a.* FROM reembolso_adjuntos a JOIN reembolsos_rendiciones r ON r.id=a.reembolso_id WHERE a.id=? AND r.obra_id=?''',(fid,active_project()['id'])).fetchone(); c.close()
    if not f: abort(404)
    return send_from_directory(UPLOADS,f['nombre_archivo'],as_attachment=False,download_name=f['nombre_original'])
...
def orders():
    u=user(); obra=active_project(); c=db()
    rows=c.execute('''SELECT o.*,u.full_name creador_nombre FROM ordenes o JOIN users u ON u.id=o.creador WHERE o.obra_id=? ORDER BY o.id DESC''',(obra['id'],)).fetchall()
    files=c.execute('''SELECT a.* FROM orden_adjuntos a JOIN ordenes o ON o.id=a.orden_id WHERE o.obra_id=? ORDER BY a.id''',(obra['id'],)).fetchall(); c.close()
...
def new_order():
    u=user(); obra=active_project()
    if u['role'] not in ('Gerencia de Obra','Administración','Sistemas'): abort(403)
    if request.method=='GET':
        c=db(); prov=c.execute('SELECT * FROM proveedores WHERE active=1 ORDER BY nombre').fetchall(); req_units,_=get_active_units(c); c.close(); return render_template('new_order.html',user=u,obra=obra,providers=prov,units=req_units)
    tipo=request.form.get('tipo_orden',''); numero=request.form.get('numero','').strip(); fecha=request.form.get('fecha',''); pname=request.form.get('proveedor_nombre','').strip(); doc=request.form.get('documento','').strip(); moneda=request.form.get('moneda','PEN'); desc=request.form.get('descripcion','').strip()
    if tipo not in ('Orden de compra','Orden de servicio') or not numero or not fecha or not pname: flash('Complete tipo, número, fecha y proveedor.','danger'); return redirect(url_for('new_order'))
    conceptos=request.form.getlist('descripcion_item[]'); unds=request.form.getlist('unidad_item[]'); qtys=request.form.getlist('cantidad_item[]'); prices=request.form.getlist('precio_item[]'); its=[]
    for i,x in enumerate(conceptos):
        if not x.strip(): continue
        q=parse_decimal(qtys[i]); p=parse_decimal(prices[i]); its.append((x.strip(),unds[i],q,p,q*p))
    if not its: flash('Agregue al menos un detalle.','danger'); return redirect(url_for('new_order'))
    c=db(); now=datetime.now().strftime('%Y-%m-%d %H:%M:%S'); total=sum(x[4] for x in its); cur=c.execute('''INSERT INTO ordenes(obra_id,tipo_orden,numero,fecha,proveedor_nombre,documento,moneda,descripcion,total,estado,creador,creado) VALUES(?,?,?,?,?,?,?,?,?,'Pendiente de Gerencia de Obra',?,?)''',(obra['id'],tipo,numero,fecha,pname,doc,moneda,desc,total,u['id'],now)); oid=cur.lastrowid
    for n,x in enumerate(its,1): c.execute('INSERT INTO orden_items(orden_id,nro,descripcion,unidad,cantidad,precio_unitario,monto) VALUES(?,?,?,?,?,?,?)',(oid,n,*x))
    for f in request.files.getlist('documentos'):
        if f and f.filename:
            guard=secure_filename(f'ORD_{oid}_{uuid.uuid4().hex[:8]}_{f.filename}'); f.save(os.path.join(UPLOADS,guard)); c.execute('INSERT INTO orden_adjuntos(orden_id,nombre_original,nombre_archivo,creado) VALUES(?,?,?,?)',(oid,f.filename,guard,now))
    c.commit(); c.close(); flash('Orden registrada. El formato oficial de exportación queda pendiente del modelo LANR.','success'); return redirect(url_for('orders'))

@app.route('/orders/files/<int:fid>')
@login_required
def order_file(fid):
    c=db()
    f=c.execute("SELECT a.* FROM orden_adjuntos a JOIN ordenes o ON o.id=a.orden_id WHERE a.id=? AND o.obra_id=?",(fid,active_project()['id'])).fetchone()
    c.close()
    if not f: abort(404)
    return send_from_directory(UPLOADS,f['nombre_archivo'],as_attachment=False,download_name=f['nombre_original'])
...
def approve_order(oid):
    u=user(); obra=active_project(); c=db()
    o=c.execute('SELECT * FROM ordenes WHERE id=? AND obra_id=?',(oid,obra['id'])).fetchone()
    if not o:
        c.close(); abort(404)
    now=datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    if o['estado']=='Pendiente de Gerencia de Obra' and u['role']=='Gerencia de Obra':
        c.execute("UPDATE ordenes SET estado='Pendiente de Administración',vb_gerencia_obra_por=?,vb_gerencia_obra_fecha=? WHERE id=?",(u['id'],now,oid)); msg='V°B° de Gerencia de Obra registrado.'
    elif o['estado']=='Pendiente de Administración' and u['role']=='Administración':
        c.execute("UPDATE ordenes SET estado='Pendiente de Gerencia General',vb_administracion_por=?,vb_administracion_fecha=? WHERE id=?",(u['id'],now,oid)); msg='V°B° de Administración registrado.'
    elif o['estado']=='Pendiente de Gerencia General' and u['role']=='Gerencia General':
        c.execute("UPDATE ordenes SET estado='Aprobada',vb_gerencia_general_por=?,vb_gerencia_general_fecha=? WHERE id=?",(u['id'],now,oid)); msg='Orden aprobada por Gerencia General.'
    else:
        c.close(); abort(403)
    c.commit(); c.close(); flash(msg,'success'); return redirect(url_for('orders'))

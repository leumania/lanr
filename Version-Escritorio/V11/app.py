...
def init_db():
    ...
    columnas_compras = [row['name'] for row in c.execute(
        "PRAGMA table_info(compras_logistica)").fetchall()]
    for columna, definicion in (
        ('proveedor_id', 'INTEGER'),
        ('solicitud_tesoreria_id', 'INTEGER'),
    ):
        if columna not in columnas_compras:
            c.execute(
                f'ALTER TABLE compras_logistica ADD COLUMN {columna} {definicion}')

    c.execute('''
        CREATE TABLE IF NOT EXISTS solicitud_tesoreria_items(
            id INTEGER PRIMARY KEY,
            solicitud_id INTEGER NOT NULL,
            autorizacion_item_id INTEGER NOT NULL,
            item_id INTEGER NOT NULL,
            cantidad REAL NOT NULL DEFAULT 0,
            precio_unitario REAL NOT NULL DEFAULT 0,
            subtotal REAL NOT NULL DEFAULT 0
    ...
    c.execute('''
        INSERT INTO solicitud_tesoreria_items(
            solicitud_id,autorizacion_item_id,item_id,cantidad,precio_unitario,subtotal
        )
        SELECT s.id,ai.id,ai.item_id,ai.cantidad,ai.precio_unitario,ai.subtotal
        FROM solicitudes_tesoreria s
        JOIN autorizacion_items ai
          ON ai.autorizacion_id=s.autorizacion_id
         AND ai.proveedor_id=s.proveedor_id
        WHERE s.origen='REQ-Cotizacion'
          AND NOT EXISTS(
              SELECT 1 FROM solicitud_tesoreria_items sti
              WHERE sti.solicitud_id=s.id AND sti.autorizacion_item_id=ai.id
          )
    ''')

    c.execute('''
        CREATE TABLE IF NOT EXISTS despachos_logistica(
            id INTEGER PRIMARY KEY,
            tramite_id INTEGER NOT NULL,
            medio_envio TEXT NOT NULL,
            responsable_transporte TEXT,
...
def refresh_req_logistics_state(c, tid):
    tramite = c.execute(
        "SELECT estado FROM tramites WHERE id=? AND tipo='REQ'", (tid,)
    ).fetchone()
    if not tramite or tramite['estado'] == 'Cerrado':
        return tramite['estado'] if tramite else None

    requeridos = c.execute(
        "SELECT id,COALESCE(comprar,0) cantidad FROM items WHERE tramite_id=?",
        (tid,)
    ).fetchall()
    requeridos = {r['id']: float(r['cantidad'] or 0) for r in requeridos if float(r['cantidad'] or 0) > 0}

    cotizaciones = c.execute(
        "SELECT estado,COUNT(*) n FROM cotizaciones WHERE tramite_id=? GROUP BY estado",
        (tid,)
    ).fetchall()
    cot_por_estado = {r['estado']: r['n'] for r in cotizaciones}

    autorizados_rows = c.execute('''
        SELECT ai.item_id,COALESCE(SUM(ai.cantidad),0) cantidad
        FROM autorizacion_items ai
        JOIN autorizaciones_compra a ON a.id=ai.autorizacion_id
        WHERE a.tramite_id=? AND a.estado='Autorizada'
        GROUP BY ai.item_id
    ''', (tid,)).fetchall()
    autorizados = {r['item_id']: float(r['cantidad'] or 0) for r in autorizados_rows}

    pagos_pendientes = c.execute('''
        SELECT COUNT(*) n
        FROM solicitudes_tesoreria s
        LEFT JOIN autorizaciones_compra a ON a.id=s.autorizacion_id
        WHERE s.tramite_id=? AND s.origen='REQ-Cotizacion'
          AND s.estado='Pendiente'
          AND (a.id IS NULL OR a.estado='Autorizada')
    ''', (tid,)).fetchone()['n']
    pagos_atendidos = c.execute('''
        SELECT COUNT(*) n FROM solicitudes_tesoreria
        WHERE tramite_id=? AND origen='REQ-Cotizacion' AND estado='Atendido'
    ''', (tid,)).fetchone()['n']

    comprados_rows = c.execute('''
        SELECT item_id,COALESCE(SUM(cantidad_comprada),0) cantidad
        FROM compra_detalle WHERE tramite_id=? GROUP BY item_id
    ''', (tid,)).fetchall()
    comprados = {r['item_id']: float(r['cantidad'] or 0) for r in comprados_rows}

    despachados_rows = c.execute('''
        SELECT item_id,COALESCE(SUM(cantidad),0) cantidad
        FROM despacho_detalle WHERE tramite_id=? GROUP BY item_id
    ''', (tid,)).fetchall()
    despachados = {r['item_id']: float(r['cantidad'] or 0) for r in despachados_rows}

    def completo(valores):
        return bool(requeridos) and all(valores.get(item_id, 0) + 0.0001 >= qty for item_id, qty in requeridos.items())

    if completo(despachados):
        estado = 'Enviado a obra'
    elif completo(comprados):
        estado = 'Listo para despacho'
    elif any(qty > 0 for qty in comprados.values()):
        estado = 'Compra/comprobantes'
    elif pagos_pendientes:
        estado = 'Pago en proceso' if pagos_atendidos else 'Pendiente de pago'
    elif pagos_atendidos:
        estado = 'Compra/comprobantes'
    elif any(qty > 0 for qty in autorizados.values()):
        estado = 'En gestión de compra'
    elif cot_por_estado.get('Enviada a Sara', 0):
        estado = 'Pendiente de autorización'
    elif cot_por_estado:
        estado = 'Cotizaciones en gestión'
    else:
...
def home():
        ...
        pendientes_logistica = c.execute(
            ...
            SELECT
                ...
                CASE
                    ...
                    WHEN t.estado IN ('Pendiente de cotización','Cotizaciones en gestión','Recibido por Logística')
                        THEN 'Cotizaciones'

                    WHEN t.estado = 'Pendiente de autorización'
                        THEN 'Autorización de Sara'

                    WHEN t.estado IN ('En gestión de compra','Pendiente de pago','Pago en proceso')
                        THEN 'Pago por proveedor'

                    WHEN t.estado = 'Compra/comprobantes'
                        THEN 'Comprobantes de compra'

                    WHEN t.estado = 'Listo para despacho'
                        THEN 'Despacho a obra'
    ...
    if role == 'Logística':
        pendientes_envio = sum(
            ...
            if x['pendiente_de'] == 'Despacho a obra'
        )
    ...
    if role == 'Tesorería':
        ...
        pendientes_tesoreria = c.execute(
            ...
            AND s.origen IN ('REQ-Compra','REQ-Cotizacion')
    ...
    if role == 'Administración':
        req_pendientes_autorizacion = c.execute(
            '''
            SELECT t.id,t.tracking,t.numero,t.fecha,t.estado,
                   creador.full_name AS creador_nombre,
                   (SELECT COUNT(*) FROM cotizaciones q WHERE q.tramite_id=t.id AND q.estado='Enviada a Sara') AS cotizaciones_enviadas
            FROM tramites t
            JOIN users creador ON creador.id=t.creador
            WHERE t.tipo='REQ' AND t.estado='Pendiente de autorización'
            ORDER BY t.id DESC
            '''
        ).fetchall()
    ...
    return render_template(
        ...
        req_pendientes_autorizacion=req_pendientes_autorizacion
    )
...
def logistics_quotations(tid):
    ...
    if not t or t['tipo'] != 'REQ':
        c.close()
        abort(404)
    if u['role'] not in ('Logística', 'Administración', 'Tesorería', 'Sistemas'):
        c.close()
        abort(403)

    items = c.execute('''
        SELECT * FROM items WHERE tramite_id=?
        ORDER BY CASE COALESCE(prioridad,'Normal') WHEN 'Urgente' THEN 0 WHEN 'Prioritario' THEN 1 ELSE 2 END,
                 ...
                 fecha_requerida ASC, seccion, nro
    ''', (tid,)).fetchall()
    proveedores = c.execute("SELECT * FROM proveedores WHERE active=1 ORDER BY nombre").fetchall()
    cotizaciones = c.execute('''
        SELECT q.*,p.nombre proveedor_nombre,p.documento proveedor_documento,u.full_name creador_nombre,
        ...
        FROM cotizaciones q
        JOIN proveedores p ON p.id=q.proveedor_id
        LEFT JOIN users u ON u.id=q.creado_por
        WHERE q.tramite_id=? ORDER BY q.id DESC
    ''', (tid,)).fetchall()
    qitems = c.execute('''
        SELECT qi.*,i.descripcion,i.unidad,p.nombre proveedor_nombre,q.estado,q.tipo_sustento,q.id cotizacion_id
        FROM cotizacion_items qi
        JOIN items i ON i.id=qi.item_id
        JOIN cotizaciones q ON q.id=qi.cotizacion_id
        JOIN proveedores p ON p.id=q.proveedor_id
        WHERE q.tramite_id=? ORDER BY i.nro,q.id
    ''', (tid,)).fetchall()
    qfiles = c.execute('''
        SELECT f.* FROM cotizacion_archivos f
        JOIN cotizaciones q ON q.id=f.cotizacion_id
        WHERE q.tramite_id=? ORDER BY f.id
    ''', (tid,)).fetchall()
    autorizaciones = c.execute('''
        SELECT a.*,u.full_name autorizado_nombre,
               (SELECT COALESCE(SUM(ai.subtotal),0) FROM autorizacion_items ai WHERE ai.autorizacion_id=a.id) total
        FROM autorizaciones_compra a
        LEFT JOIN users u ON u.id=a.autorizado_por
        WHERE a.tramite_id=? ORDER BY a.id DESC
    ''', (tid,)).fetchall()
    aitems = c.execute('''
        SELECT ai.*,i.descripcion,i.unidad,p.nombre proveedor_nombre
        FROM autorizacion_items ai
        JOIN items i ON i.id=ai.item_id
        JOIN proveedores p ON p.id=ai.proveedor_id
        JOIN autorizaciones_compra a ON a.id=ai.autorizacion_id
        WHERE a.tramite_id=? ORDER BY ai.autorizacion_id,i.nro
    ''', (tid,)).fetchall()
    pagos = c.execute('''
        SELECT s.*,p.documento proveedor_documento
        FROM solicitudes_tesoreria s
        LEFT JOIN proveedores p ON p.id=s.proveedor_id
        WHERE s.tramite_id=? AND s.origen='REQ-Cotizacion'
        ORDER BY CASE WHEN s.estado='Pendiente' THEN 0 ELSE 1 END,s.id DESC
    ''', (tid,)).fetchall()
    pago_items = c.execute('''
        SELECT sti.*,i.descripcion,i.unidad
        FROM solicitud_tesoreria_items sti
        JOIN items i ON i.id=sti.item_id
        JOIN solicitudes_tesoreria s ON s.id=sti.solicitud_id
        WHERE s.tramite_id=? ORDER BY s.id,i.nro
    ''', (tid,)).fetchall()
    compras = c.execute('''
        SELECT cl.*,p.nombre proveedor_nombre
        FROM compras_logistica cl
        LEFT JOIN proveedores p ON p.id=cl.proveedor_id
        WHERE cl.tramite_id=? ORDER BY cl.id DESC
    ''', (tid,)).fetchall()
    compra_items = c.execute('''
        SELECT cd.*,i.descripcion,i.unidad,cl.proveedor_id,p.nombre proveedor_nombre
        FROM compra_detalle cd
        JOIN items i ON i.id=cd.item_id
        JOIN compras_logistica cl ON cl.id=cd.compra_id
        LEFT JOIN proveedores p ON p.id=cl.proveedor_id
        WHERE cd.tramite_id=? ORDER BY cd.id
    ''', (tid,)).fetchall()
    despachos = c.execute('''
        SELECT * FROM despachos_logistica WHERE tramite_id=? ORDER BY id DESC
    ''', (tid,)).fetchall()
    despacho_items = c.execute('''
        SELECT dd.*,i.descripcion,i.unidad
        FROM despacho_detalle dd JOIN items i ON i.id=dd.item_id
        WHERE dd.tramite_id=? ORDER BY dd.id
    ''', (tid,)).fetchall()

    auth_rows = c.execute('''
        SELECT ai.item_id,COALESCE(SUM(ai.cantidad),0) qty
        FROM autorizacion_items ai
        JOIN autorizaciones_compra a ON a.id=ai.autorizacion_id
        WHERE a.tramite_id=? AND a.estado='Autorizada'
        GROUP BY ai.item_id
    ...
    bought_rows = c.execute('''
        SELECT item_id,COALESCE(SUM(cantidad_comprada),0) qty
        FROM compra_detalle WHERE tramite_id=? GROUP BY item_id
    ''', (tid,)).fetchall()
    comprado = {r['item_id']: float(r['qty'] or 0) for r in bought_rows}
    sent_rows = c.execute('''
        SELECT item_id,COALESCE(SUM(cantidad),0) qty
        FROM despacho_detalle WHERE tramite_id=? GROUP BY item_id
    ''', (tid,)).fetchall()
    despachado = {r['item_id']: float(r['qty'] or 0) for r in sent_rows}

    c.close()
    return render_template(
        'quotation_management.html', user=u, t=t, items=items, proveedores=proveedores,
        cotizaciones=cotizaciones, qitems=qitems, qfiles=qfiles,
        autorizaciones=autorizaciones, aitems=aitems, pagos=pagos, pago_items=pago_items,
        compras=compras, compra_items=compra_items, despachos=despachos,
        despacho_items=despacho_items, autorizado=autorizado, comprado=comprado,
        despachado=despachado, fecha_hoy=datetime.now().strftime('%Y-%m-%d')
    )
...
def quotation_create(tid):
    ...
    if not t or t['tipo'] != 'REQ' or t['estado'] not in ('Recibido por Logística', 'Pendiente de cotización', 'Cotizaciones en gestión', 'Pendiente de autorización', 'En gestión de compra', 'Pendiente de pago', 'Pago en proceso', 'Compra/comprobantes'):
        c.close()
        abort(400)
    ...
    if tipo not in ('Cotización', 'Proveedor habitual'):
        c.close()
        abort(400)
    if tipo == 'Proveedor habitual' and not obs:
        c.close()
        flash('Para proveedor habitual, indique el motivo por el que no se adjunta una cotización nueva.', 'danger')
        return redirect(url_for('logistics_quotations', tid=tid))
    if not proveedor_id:
        c.close()
        flash('Seleccione un proveedor.', 'danger')
        return redirect(url_for('logistics_quotations', tid=tid))
    seleccionados = set(request.form.getlist('selected_item[]'))
    item_ids = request.form.getlist('item_id[]')
    cantidades = request.form.getlist('cantidad[]')
    precios = request.form.getlist('precio_unitario[]')
    filas = []
    for idx, item_id in enumerate(item_ids):
        if item_id not in seleccionados:
            continue
        qty = parse_decimal(cantidades[idx] if idx < len(cantidades) else '')
        price = parse_decimal(precios[idx] if idx < len(precios) else '')
        item = c.execute("SELECT id,comprar FROM items WHERE id=? AND tramite_id=?", (int(item_id), tid)).fetchone()
        if not item or qty <= 0 or price <= 0:
            c.close()
            flash('Cada producto seleccionado debe tener cantidad cotizada y precio unitario mayores que 0.', 'danger')
            return redirect(url_for('logistics_quotations', tid=tid))
        if qty > float(item['comprar'] or 0) + 0.0001:
            c.close()
            flash('La cantidad cotizada no puede superar la cantidad a comprar del requerimiento.', 'danger')
            return redirect(url_for('logistics_quotations', tid=tid))
    ...
    refresh_req_logistics_state(c, tid)
    c.execute("INSERT INTO history(tramite_id,usuario_id,accion,fecha) VALUES(?,?,?,?)",
...
def quotation_send(qid):
    ...
    refresh_req_logistics_state(c, q['tramite_id'])
    c.execute("INSERT INTO history(tramite_id,usuario_id,accion,fecha) VALUES(?,?,?,?)",
...
def quotation_authorize(tid):
    ...
    for pid, total in totals.items():
        ...
        cur_pago = c.execute("INSERT INTO solicitudes_tesoreria(tramite_id,origen,solicitado_por,motivo,monto,estado,fecha_solicitud,autorizacion_id,proveedor_id,proveedor_nombre) VALUES(?, 'REQ-Cotizacion', ?, ?, ?, 'Pendiente', ?, ?, ?, ?)",
                  (tid, u['id'], f'Compra autorizada - {prov["nombre"]}', total, now, aid, pid, prov['nombre']))
        solicitud_id = cur_pago.lastrowid
        for ai in c.execute("SELECT * FROM autorizacion_items WHERE autorizacion_id=? AND proveedor_id=? ORDER BY id", (aid, pid)).fetchall():
            c.execute("INSERT INTO solicitud_tesoreria_items(solicitud_id,autorizacion_item_id,item_id,cantidad,precio_unitario,subtotal) VALUES(?,?,?,?,?,?)",
                      (solicitud_id, ai['id'], ai['item_id'], ai['cantidad'], ai['precio_unitario'], ai['subtotal']))
    refresh_req_logistics_state(c, tid)
    c.execute("INSERT INTO history(tramite_id,usuario_id,accion,fecha) VALUES(?,?,?,?)",
...
def authorization_cancel(aid):
    ...
    refresh_req_logistics_state(c, a['tramite_id'])
    c.commit()
    c.close()
...
def logistics_purchase(tid):
    ...
    if u['role'] != 'Logística':
        abort(403)
    flash('La compra ahora se gestiona por proveedor desde Cotizaciones y compras.', 'warning')
    return redirect(url_for('logistics_quotations', tid=tid))
...
def logistics_receipts(tid):
    ...
    fecha = request.form.get('fecha_compra', '').strip()
    tipo = request.form.get('tipo_comprobante', '').strip()
    numero = request.form.get('nro_comprobante', '').strip()
    monto = parse_decimal(request.form.get('monto'))
    archivo = request.files.get('comprobante')
    item_ids = request.form.getlist('item_id[]')
    cantidades = request.form.getlist('cantidad_comprada[]')

    if not proveedor_id or not solicitud_id or not fecha or tipo not in ('Factura', 'Boleta', 'Otro') or not numero or monto <= 0:
        flash('Complete proveedor, pago relacionado, fecha, tipo, número y monto del comprobante.', 'danger')
        return redirect(url_for('logistics_quotations', tid=tid))
    if not archivo or not archivo.filename:
        flash('Adjunte el comprobante en PDF, imagen o XML.', 'danger')
        return redirect(url_for('logistics_quotations', tid=tid))
    if not archivo.filename.lower().endswith(('.pdf', '.png', '.jpg', '.jpeg', '.webp', '.xml')):
        flash('La evidencia debe ser PDF, imagen o XML.', 'danger')
        return redirect(url_for('logistics_quotations', tid=tid))

    c = db()
    t = c.execute("SELECT * FROM tramites WHERE id=? AND tipo='REQ'", (tid,)).fetchone()
    solicitud = c.execute('''
        SELECT * FROM solicitudes_tesoreria
        WHERE id=? AND tramite_id=? AND proveedor_id=?
          AND origen='REQ-Cotizacion' AND estado='Atendido'
    ''', (int(solicitud_id), tid, int(proveedor_id))).fetchone()
    if not t or not solicitud:
        c.close()
        flash('El comprobante debe corresponder a un proveedor con pago atendido por Tesorería.', 'danger')
        return redirect(url_for('logistics_quotations', tid=tid))

    total_comprobado = c.execute(
        "SELECT COALESCE(SUM(monto),0) total FROM compras_logistica WHERE solicitud_tesoreria_id=?",
        (int(solicitud_id),)
    ).fetchone()['total']
    if float(total_comprobado or 0) + monto > float(solicitud['monto'] or 0) + 0.01:
        c.close()
        flash('El total de comprobantes no puede superar el monto pagado a este proveedor.', 'danger')
        return redirect(url_for('logistics_quotations', tid=tid))

    autorizado_rows = c.execute('''
        SELECT sti.item_id, i.descripcion, i.unidad,
               COALESCE(SUM(sti.cantidad),0) autorizado,
               COALESCE((SELECT SUM(cd.cantidad_comprada)
                         FROM compra_detalle cd
                         JOIN compras_logistica cl ON cl.id=cd.compra_id
                         WHERE cd.tramite_id=? AND cd.item_id=sti.item_id
                           AND cl.solicitud_tesoreria_id=?),0) comprado
        FROM solicitud_tesoreria_items sti
        JOIN items i ON i.id=sti.item_id
        WHERE sti.solicitud_id=?
        GROUP BY sti.item_id,i.descripcion,i.unidad
    ''', (tid, int(solicitud_id), int(solicitud_id))).fetchall()
    permitido = {
        r['item_id']: (
            float(r['autorizado'] or 0),
            float(r['comprado'] or 0),
            r['descripcion']
        ) for r in autorizado_rows
    }

    detalles = []
    for idx, item_id_txt in enumerate(item_ids):
        try:
        ...
        if qty <= 0:
            continue
        if item_id not in permitido:
            c.close()
            flash('Uno de los productos no pertenece al pago seleccionado.', 'danger')
            return redirect(url_for('logistics_quotations', tid=tid))
        autorizado, comprado, descripcion = permitido[item_id]
        if comprado + qty > autorizado + 0.0001:
            c.close()
            flash(f'La cantidad comprada de "{descripcion}" supera lo autorizado y pagado para este proveedor.', 'danger')
            return redirect(url_for('logistics_quotations', tid=tid))
        detalles.append((item_id, qty))

    if not detalles:
        c.close()
        flash('Registre al menos una cantidad comprada para este comprobante.', 'danger')
        return redirect(url_for('logistics_quotations', tid=tid))

    now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    original = archivo.filename
    guardado = secure_filename(
        f'COMPRA_{tid}_{proveedor_id}_{datetime.now().strftime("%Y%m%d%H%M%S%f")}_{archivo.filename}'
    )
    archivo.save(os.path.join(UPLOADS, guardado))
    cur = c.execute('''
        INSERT INTO compras_logistica(
            tramite_id,fecha_compra,tipo_comprobante,nro_comprobante,monto,
            nombre_original,nombre_archivo,creado,proveedor_id,solicitud_tesoreria_id
        ) VALUES(?,?,?,?,?,?,?,?,?,?)
    ''', (tid, fecha, tipo, numero, monto, original, guardado, now, int(proveedor_id), int(solicitud_id)))
    compra_id = cur.lastrowid
    for item_id, qty in detalles:
        c.execute('''
            INSERT INTO compra_detalle(tramite_id,item_id,compra_id,cantidad_comprada,creado)
            VALUES(?,?,?,?,?)
        ''', (tid, item_id, compra_id, qty, now))

    c.execute('INSERT OR IGNORE INTO gestion_logistica(tramite_id) VALUES(?)', (tid,))
    c.execute('''
        UPDATE gestion_logistica
        SET fecha_compra=?,tipo_comprobante=?,nro_comprobante=?,comprobante_pendiente=0
        WHERE tramite_id=?
    ''', (fecha, tipo, numero, tid))
    refresh_req_logistics_state(c, tid)
    prov = c.execute("SELECT nombre FROM proveedores WHERE id=?", (int(proveedor_id),)).fetchone()
    proveedor_nombre = prov['nombre'] if prov else 'proveedor'
    c.execute('''
        INSERT INTO history(tramite_id,usuario_id,accion,fecha)
        VALUES(?,?,?,?)
    ''', (tid, u['id'], f'Comprobante {numero} registrado para {proveedor_nombre} - S/ {monto:,.2f}', now))
    c.commit()
    c.close()
    flash('Comprobante y cantidades compradas registrados correctamente.', 'success')
    return redirect(url_for('logistics_quotations', tid=tid))
...
def logistics_payment(tid):
    ...
    if u['role'] != 'Logística':
        abort(403)
    flash('Los pagos ahora se gestionan por proveedor desde Cotizaciones y compras.', 'warning')
    return redirect(url_for('logistics_quotations', tid=tid))


@app.route('/logistics/provider-payment/<int:sid>', methods=['POST'])
@login_required
def logistics_provider_payment(sid):
    u = user()
    if u['role'] != 'Logística':
        abort(403)
    forma = request.form.get('forma_pago', '').strip()
    fecha = request.form.get('fecha_pago', '').strip() or datetime.now().strftime('%Y-%m-%d')
    evidencia = request.files.get('comprobante_pago')
    if forma not in ('Caja Logística', 'Pago personal'):
        flash('Seleccione una forma de pago válida.', 'danger')
        return redirect(request.referrer or url_for('home'))

    c = db()
    solicitud = c.execute('''
        SELECT s.*,t.tracking FROM solicitudes_tesoreria s
        JOIN tramites t ON t.id=s.tramite_id
        WHERE s.id=? AND s.origen='REQ-Cotizacion'
    ''', (sid,)).fetchone()
    if not solicitud or solicitud['estado'] != 'Pendiente':
        c.close()
        flash('Este pago ya fue atendido o no está disponible.', 'warning')
        return redirect(request.referrer or url_for('home'))
    if float(solicitud['monto'] or 0) >= 1900:
        c.close()
        flash('Los pagos de S/ 1,900.00 o más deben ser atendidos por Tesorería.', 'danger')
        return redirect(url_for('logistics_quotations', tid=solicitud['tramite_id']))
    if evidencia and evidencia.filename and not evidencia.filename.lower().endswith(('.pdf', '.png', '.jpg', '.jpeg', '.webp')):
        c.close()
        flash('La evidencia debe ser PDF o imagen.', 'danger')
        return redirect(url_for('logistics_quotations', tid=solicitud['tramite_id']))

    now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    if evidencia and evidencia.filename:
        original = evidencia.filename
        guardado = secure_filename(f'PAGO_LOG_{sid}_{datetime.now().strftime("%Y%m%d%H%M%S%f")}_{evidencia.filename}')
        evidencia.save(os.path.join(UPLOADS, guardado))
        c.execute('''
            INSERT INTO pagos_tesoreria(solicitud_id,medio_pago,banco,monto,nro_operacion,nombre_original,nombre_archivo,fecha)
            VALUES(?,?,?,?,?,?,?,?)
        ''', (sid, forma, None, solicitud['monto'], None, original, guardado, fecha))

    c.execute("UPDATE solicitudes_tesoreria SET estado='Atendido',fecha_atencion=? WHERE id=?", (now, sid))
    if forma == 'Pago personal':
        c.execute('''
            INSERT INTO solicitudes_tesoreria(tramite_id,origen,solicitado_por,motivo,monto,estado,fecha_solicitud,proveedor_id,proveedor_nombre)
            VALUES(?, 'REQ-Reembolso', ?, ?, ?, 'Pendiente', ?, ?, ?)
        ''', (solicitud['tramite_id'], u['id'], f'Reembolso de pago a {solicitud["proveedor_nombre"] or "proveedor"}', solicitud['monto'], now, solicitud['proveedor_id'], solicitud['proveedor_nombre']))

    pendientes = c.execute('''
        SELECT COUNT(*) n FROM solicitudes_tesoreria
        WHERE tramite_id=? AND origen='REQ-Cotizacion' AND estado='Pendiente'
    ''', (solicitud['tramite_id'],)).fetchone()['n']
    c.execute('INSERT OR IGNORE INTO gestion_logistica(tramite_id) VALUES(?)', (solicitud['tramite_id'],))
    c.execute("UPDATE gestion_logistica SET estado_pago=? WHERE tramite_id=?",
              ('Pago en proceso' if pendientes else 'Pagos atendidos', solicitud['tramite_id']))
    refresh_req_logistics_state(c, solicitud['tramite_id'])
    c.execute('INSERT INTO history(tramite_id,usuario_id,accion,fecha) VALUES(?,?,?,?)',
              (solicitud['tramite_id'], u['id'], f'Pago a {solicitud["proveedor_nombre"] or "proveedor"} atendido por Logística mediante {forma}', now))
    yoana = c.execute("SELECT id FROM users WHERE role='Tesorería' AND active=1 ORDER BY id LIMIT 1").fetchone()
    c.commit()
    c.close()
    if yoana and forma == 'Pago personal':
        notify(yoana['id'], solicitud['tramite_id'], 'Reembolso pendiente',
               f'Logística registró un pago personal de S/ {float(solicitud["monto"] or 0):,.2f} para {solicitud["tracking"]}.', 'accion')
    flash('Pago del proveedor registrado correctamente.', 'success')
    return redirect(url_for('logistics_quotations', tid=solicitud['tramite_id']))
...
def treasury_detail(sid):
    ...
    if not solicitud:
        c.close()
        abort(404)
    compras = c.execute("""SELECT cl.*,p.nombre proveedor_nombre FROM compras_logistica cl
        LEFT JOIN proveedores p ON p.id=cl.proveedor_id
        WHERE cl.tramite_id=? AND (cl.solicitud_tesoreria_id=? OR cl.solicitud_tesoreria_id IS NULL) ORDER BY cl.id""",
                        (solicitud['tramite_id'], sid)).fetchall()
    productos_pago = c.execute("""
        SELECT sti.*,i.descripcion,i.unidad
        FROM solicitud_tesoreria_items sti
        JOIN items i ON i.id=sti.item_id
        WHERE sti.solicitud_id=? ORDER BY i.nro,sti.id
    """, (sid,)).fetchall()
    if not productos_pago and solicitud['autorizacion_id'] and solicitud['proveedor_id']:
        productos_pago = c.execute("""
            SELECT ai.item_id,ai.cantidad,ai.precio_unitario,ai.subtotal,i.descripcion,i.unidad
            FROM autorizacion_items ai
            JOIN items i ON i.id=ai.item_id
            WHERE ai.autorizacion_id=? AND ai.proveedor_id=? ORDER BY i.nro,ai.id
        """, (solicitud['autorizacion_id'], solicitud['proveedor_id'])).fetchall()
    pagos = c.execute(
        "SELECT * FROM pagos_tesoreria WHERE solicitud_id=? ORDER BY id", (sid,)).fetchall()
    c.close()
    return render_template('treasury_payment_detail.html', user=u, solicitud=solicitud, compras=compras, productos_pago=productos_pago, pagos=pagos, fecha_hoy=datetime.now().strftime('%Y-%m-%d'))
...
def treasury_pay(sid):
    ...
    if solicitud['origen'] in ('REQ-Compra', 'REQ-Cotizacion'):
        ...
        if pendientes == 0:
            ...
            c.execute(
                "UPDATE gestion_logistica SET estado_pago='Pago en proceso' WHERE tramite_id=?", (tid,))
        refresh_req_logistics_state(c, tid)
...
def logistics_dispatch(tid):
    ...
    responsable_transporte = request.form.get('responsable_transporte', '').strip()
    costo_envio = parse_decimal(request.form.get('costo_envio'))
    observacion_envio = request.form.get('observacion_envio', '').strip()
    guia_numero = request.form.get('guia_numero', '').strip()
    guia_fecha = request.form.get('guia_fecha', '').strip()
    guia_opcion = request.form.get('guia_opcion', '').strip()
    guia_pendiente = 1 if guia_opcion == 'pendiente' else 0
    item_ids = request.form.getlist('dispatch_item_id[]')
    cantidades = request.form.getlist('cantidad_despachada[]')

    archivos_guia = [f for f in request.files.getlist('guia') if f and f.filename]
    comprobantes_transporte = [f for f in request.files.getlist('comprobante_transporte') if f and f.filename]
    evidencias_envio = [f for f in request.files.getlist('evidencia_envio') if f and f.filename]

    if medio_envio not in ('Chofer de LANR', 'Encomienda', 'Transporte del proveedor', 'Otro'):
        flash('Seleccione un medio de envío.', 'danger')
        return redirect(url_for('logistics_quotations', tid=tid))
    if costo_envio < 0:
        flash('El costo adicional de envío no puede ser negativo.', 'danger')
        return redirect(url_for('logistics_quotations', tid=tid))
    if not evidencias_envio:
        flash('Debes adjuntar al menos una evidencia del envío.', 'danger')
        return redirect(url_for('logistics_quotations', tid=tid))
    if any(not f.filename.lower().endswith(('.jpg', '.jpeg', '.png', '.webp')) for f in evidencias_envio):
        flash('La evidencia del envío debe ser una imagen JPG, JPEG, PNG o WEBP.', 'danger')
        return redirect(url_for('logistics_quotations', tid=tid))
    ...
    if guia_opcion == 'adjuntar':
        if not guia_numero or not guia_fecha or not archivos_guia:
            flash('Para adjuntar la guía, complete número, fecha y archivo.', 'danger')
            return redirect(url_for('logistics_quotations', tid=tid))
    else:
        guia_numero = ''
        guia_fecha = ''
    ...
    if any(not f.filename.lower().endswith(permitidos) for f in archivos_guia + comprobantes_transporte):
        flash('La guía y el comprobante de transporte deben ser PDF o imagen.', 'danger')
        return redirect(url_for('logistics_quotations', tid=tid))
    ...
    if t['estado'] == 'Cerrado':
        c.close()
        flash('Este requerimiento ya está finalizado.', 'warning')
        return redirect(url_for('request_detail', tid=tid))

    disponibles_rows = c.execute('''
        SELECT i.id,i.descripcion,i.unidad,
               COALESCE((SELECT SUM(cd.cantidad_comprada) FROM compra_detalle cd WHERE cd.tramite_id=? AND cd.item_id=i.id),0) comprado,
               COALESCE((SELECT SUM(dd.cantidad) FROM despacho_detalle dd WHERE dd.tramite_id=? AND dd.item_id=i.id),0) despachado
        FROM items i WHERE i.tramite_id=?
    ''', (tid, tid, tid)).fetchall()
    disponibles = {
        r['id']: (float(r['comprado'] or 0), float(r['despachado'] or 0), r['descripcion'])
        for r in disponibles_rows
    }
    detalles = []
    for idx, item_id_txt in enumerate(item_ids):
        try:
        ...
        if qty <= 0:
            continue
        if item_id not in disponibles:
            c.close()
            flash('Uno de los productos del despacho no pertenece al requerimiento.', 'danger')
            return redirect(url_for('logistics_quotations', tid=tid))
        comprado, despachado, descripcion = disponibles[item_id]
        pendiente = max(0, comprado - despachado)
        if qty > pendiente + 0.0001:
            c.close()
            flash(f'La cantidad a despachar de "{descripcion}" supera lo comprado pendiente de envío.', 'danger')
            return redirect(url_for('logistics_quotations', tid=tid))
        detalles.append((item_id, qty))
    if not detalles:
        c.close()
        flash('Seleccione al menos un producto con cantidad para despachar.', 'danger')
        return redirect(url_for('logistics_quotations', tid=tid))

    save_logistics_files(c, tid, archivos_guia, 'guia')
    save_logistics_files(c, tid, comprobantes_transporte, 'comprobante_transporte')
    save_logistics_files(c, tid, evidencias_envio, 'evidencia_envio')
    now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    cur = c.execute('''
        INSERT INTO despachos_logistica(
            tramite_id,medio_envio,responsable_transporte,costo_envio,observacion,
            guia_numero,guia_fecha,guia_pendiente,creado_por,creado
        ) VALUES(?,?,?,?,?,?,?,?,?,?)
    ''', (tid, medio_envio, responsable_transporte or None, costo_envio, observacion_envio or None,
          guia_numero or None, guia_fecha or None, guia_pendiente, u['id'], now))
    despacho_id = cur.lastrowid
    for item_id, qty in detalles:
        c.execute('''
            INSERT INTO despacho_detalle(despacho_id,tramite_id,item_id,cantidad,creado)
            VALUES(?,?,?,?,?)
        ''', (despacho_id, tid, item_id, qty, now))

    c.execute('INSERT OR IGNORE INTO gestion_logistica(tramite_id) VALUES(?)', (tid,))
    c.execute('''
        UPDATE gestion_logistica
        SET medio_envio=?,responsable_transporte=?,costo_envio=?,observacion_envio=?,
            guia_numero=?,guia_fecha=?,guia_pendiente=?,enviado_fecha=?
        WHERE tramite_id=?
    ''', (medio_envio, responsable_transporte, costo_envio, observacion_envio,
          guia_numero, guia_fecha, guia_pendiente, now, tid))
    if guia_pendiente:
        create_regularization(c, tid, u['id'], 'Guía de remisión', 'Regularizar la guía de remisión correspondiente al envío a obra.')

    estado = refresh_req_logistics_state(c, tid)
    descripcion_historial = f'Despacho #{despacho_id} registrado mediante {medio_envio}'
    if responsable_transporte:
        descripcion_historial += f' - {responsable_transporte}'
    c.execute('INSERT INTO history(tramite_id,usuario_id,accion,fecha) VALUES(?,?,?,?)',
              (tid, u['id'], descripcion_historial, now))
    destinatarios_obra = c.execute("SELECT id FROM users WHERE role IN ('Gerencia de Obra','Control y Planeamiento') AND active=1").fetchall()
    tracking = t['tracking']
    c.commit()
    c.close()

    titulo = 'Requerimiento enviado a obra' if estado == 'Enviado a obra' else 'Despacho parcial enviado a obra'
    mensaje = (f'El requerimiento {tracking} fue enviado completamente a obra.' if estado == 'Enviado a obra'
               else f'Se registró un despacho parcial del requerimiento {tracking}.')
    for destinatario in destinatarios_obra:
        notify(destinatario['id'], tid, titulo, mensaje, 'accion')
    flash('Despacho registrado correctamente.' + (' El requerimiento quedó enviado a obra.' if estado == 'Enviado a obra' else ' Aún quedan cantidades pendientes.'), 'success')
    return redirect(url_for('logistics_quotations', tid=tid))

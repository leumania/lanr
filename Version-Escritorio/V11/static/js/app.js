...
(function () {
  ...
  function unitOptionsHtml(units, selected, kind) {
    ...
    var placeholder = kind === 'req' ? 'Seleccione' : 'Unidad';
  ...
  function reqShowImageLimit(detailRow, maxImages) {
    var message = "Este ítem admite como máximo " + maxImages + " imágenes de referencia. Quite una imagen si desea agregar otra.";
    if (document.getElementById("reqItemGuardModal")) {
      reqOpenGuardModal({ title: "Máximo de imágenes", message: message });
      return;
    }
    if (typeof editReqShowGuard === "function") {
      editReqShowGuard(message, "info");
      return;
    }
    window.alert(message);
  }

  function reqSetInputFiles(input, files) {
    if (!input || typeof DataTransfer === "undefined") return false;
    var dt = new DataTransfer();
    files.forEach(function (file) { dt.items.add(file); });
    input.files = dt.files;
    input._lanrPendingFiles = files.slice();
    return true;
  }

  function reqFileIdentity(file) {
  ...
  function reqRenderSelectedImages(detailRow) {
    ...
    var picker = detailRow.querySelector('.lanr-image-picker');
    ...
    var files = Array.from(input._lanrPendingFiles || input.files || []);
    files.forEach(function (file, index) {
      ...
      thumb.innerHTML = '<img src="' + url + '" alt="Nueva referencia">' +
        '<button type="button" class="req-image-remove-new" data-file-index="' + index + '" title="Quitar imagen" aria-label="Quitar imagen"><i class="fa-solid fa-xmark"></i></button>';
    ...
    if (picker) picker.classList.toggle('limit-reached', total >= 5);
  }
  ...
  function reqRemoveSelectedFile(detailRow, index) {
    ...
    if (!reqSetInputFiles(input, files)) return;
  ...
  function reqSetupDetails(mainRow, detailRow) {
    ...
    if (fileInput) {
      fileInput._lanrPendingFiles = Array.from(fileInput.files || []);
      fileInput.addEventListener('change', function () {
        var nuevos = Array.from(fileInput.files || []);
        var acumulados = Array.from(fileInput._lanrPendingFiles || []);
        var existentesVisible = detailRow.querySelectorAll('.req-image-thumb.existing:not(.marked-remove)').length;
        var disponibles = Math.max(5 - existentesVisible - acumulados.length, 0);
        var ids = new Set(acumulados.map(reqFileIdentity));
        var aceptados = [];

        nuevos.forEach(function (file) {
          var id = reqFileIdentity(file);
          if (!ids.has(id) && aceptados.length < disponibles) {
            aceptados.push(file);
            ids.add(id);
          }
        });

        var excedio = nuevos.some(function (file) {
          return !acumulados.some(function (oldFile) { return reqFileIdentity(oldFile) === reqFileIdentity(file); });
        }) && nuevos.filter(function (file) { return !acumulados.some(function (oldFile) { return reqFileIdentity(oldFile) === reqFileIdentity(file); }); }).length > aceptados.length;

        var combinados = acumulados.concat(aceptados);
        reqSetInputFiles(fileInput, combinados);
        reqRenderSelectedImages(detailRow);
        reqRefreshDetailsSummary(mainRow, detailRow);

        if (excedio || (disponibles === 0 && nuevos.length)) {
          reqShowImageLimit(detailRow, 5);
        }
      });
    }
  ...
  function buildReqRowCompat(seccion) {
    ...
    tr.innerHTML =
      ...
      '</td>' +
      '<td class="nt-td-cant"><input type="number" step="0.01" min="0.01" name="cantidad[]" required value="" placeholder="Cantidad"></td>' +
      ...
      '<td class="nt-td-buy"><input class="ro buy" readonly value=""></td>' +
      ...
      '<td colspan="9"><div class="req-details-panel">' +
        ...
        '<div class="req-details-grid">' +
          '<div class="req-detail-field"><label>Prioridad</label>' +
            ...
            '<small>Normal por defecto. Cámbiela solo si corresponde.</small></div>' +
          ...
          '<div class="req-detail-field req-detail-images"><label title="Opcional. Adjunte imágenes que ayuden a identificar el material.">Imágenes de referencia <span>(opcional)</span></label>' +
            '<label class="req-image-btn lanr-image-picker"><i class="fa-regular fa-images"></i><span>Adjuntar imágenes</span><small>JPG, PNG o WEBP · Máx. 5</small>' +
            '<input class="lanr-file-input-hidden" type="file" name="imagenes_' + itemKey + '" accept="image/jpeg,image/png,image/webp" multiple></label>' +
            '<span class="req-image-count">Sin imágenes</span><div class="req-image-preview" data-image-preview></div>' +
            '<small>JPG, PNG o WEBP. Puede agregar imágenes en varias selecciones hasta completar 5.</small></div>' +
        '</div>' +
      '</div></td>';
    ...
    function calc() {
      ...
      var v = Math.max(q - st, 0);
      comprar.value = Number.isInteger(v) ? String(v) : String(v).replace(/0+$/, "").replace(/\.$/, "");
    }
...
(function(){
 const tipo=document.getElementById('tramiteTipoV2'), q=document.getElementById('requestSearchV2');
 const rows=Array.from(document.querySelectorAll('.tramite-row')), empty=document.getElementById('requestsEmptyRow');
 const emptyMessage=document.getElementById('requestsEmptyMessage');
 const tabs=Array.from(document.querySelectorAll('.tramite-status-tab'));
 const note=document.getElementById('tramiteTabNote');
 const pagination=document.getElementById('tramitePagination');
 const paginationInfo=document.getElementById('tramitePaginationInfo');
 const pageNumbers=document.getElementById('tramitePageNumbers');
 const prevPage=document.getElementById('tramitePrevPage');
 const nextPage=document.getElementById('tramiteNextPage');
 const n=v=>(v||'').toString().trim().toLowerCase();
 const soloNumeros=v=>n(v).replace(/\D/g,'');
 const PAGE_SIZE=7;
 let paginaActual=1;
 const numeroBuscable=(valor, tipoCodigo)=>{
   const texto=n(valor);
   if(!texto) return '';
   const partes=texto.split('-').filter(Boolean);
   let candidato='';
   if(tipoCodigo==='seguimiento') candidato=partes.length ? partes[partes.length-1] : texto;
   else if(tipoCodigo==='numero') candidato=partes.length ? (partes[0].length===4 && partes.length>1 ? partes[partes.length-1] : partes[0]) : texto;
   else candidato=soloNumeros(texto);
   const digitos=soloNumeros(candidato);
   if(!digitos) return '';
 ...
 function actualizarContadores(){
   const counts={pendientes:0,aprobar:0,proceso:0,finalizados:0,todos:rows.length};
   rows.forEach(row=>{ const f=row.dataset.flujo||'proceso'; if(Object.prototype.hasOwnProperty.call(counts,f)) counts[f]++; });
   const map={pendientes:'countPendientes',aprobar:'countAprobar',proceso:'countProceso',finalizados:'countFinalizados',todos:'countTodos'};
   Object.keys(map).forEach(k=>{ const el=document.getElementById(map[k]); if(el) el.textContent=counts[k]; });
   if(counts.pendientes===0){
     if(counts.aprobar>0) flujoActivo='aprobar';
     else if(counts.proceso>0) flujoActivo='proceso';
     else if(counts.finalizados>0) flujoActivo='finalizados';
     else flujoActivo='todos';
   }
 }

 function filasCoincidentes(){
   const tv=n(tipo.value);
   const text=soloNumeros(q.value);
   const buscado=text ? String(parseInt(text,10)) : '';
   return rows.filter(row=>{
     ...
     const seguimiento=numeroBuscable(row.dataset.seguimiento,'seguimiento');
     const numero=numeroBuscable(row.dataset.numero,'numero');
     const formato=numeroBuscable(row.dataset.formato,'formato');
     const okTexto=!text || seguimiento===buscado || numero===buscado || formato===buscado;
     return okFlujo && okTipo && okTexto;
   });
 }
 ...
 function renderPaginacion(total){
   ...
   paginationInfo.textContent=`Mostrando ${inicio} a ${fin} de ${total} ${total===1?'trámite':'trámites'}`;
   prevPage.disabled=paginaActual<=1;
   nextPage.disabled=paginaActual>=paginas;
   pageNumbers.innerHTML='';
   for(let i=1;i<=paginas;i++){
     const b=document.createElement('button');
     b.type='button'; b.className='tramite-page-btn'+(i===paginaActual?' active':''); b.textContent=i;
     b.setAttribute('aria-label',`Página ${i}`);
     b.addEventListener('click',()=>{ paginaActual=i; filtrar(false); });
     pageNumbers.appendChild(b);
   }
 }

 function filtrar(resetPage=true){
   if(resetPage) paginaActual=1;
   const matches=filasCoincidentes();
   const total=matches.length;
   const paginas=Math.max(1,Math.ceil(total/PAGE_SIZE));
   if(paginaActual>paginas) paginaActual=paginas;
   const desde=(paginaActual-1)*PAGE_SIZE;
   const visiblesPagina=new Set(matches.slice(desde,desde+PAGE_SIZE));
   rows.forEach(row=>{ row.style.display=visiblesPagina.has(row)?'':'none'; });
   empty.style.display=total===0?'table-row':'none';
   if(total===0 && emptyMessage){
     emptyMessage.textContent=(tipo.value || q.value.trim()) ? 'No se encontraron trámites con la búsqueda realizada.' : 'No hay trámites en esta categoría.';
   }
 ...
 function limpiarFiltros(enfocar=false){
   tipo.value=''; q.value=''; paginaActual=1;
   if(enfocar) q.focus();
 }

 const btn=document.getElementById('btnBuscarTramite');
 const limpiar=document.getElementById('btnLimpiarTramite');
 function ejecutarBusqueda(){ q.value=q.value.replace(/\D/g,''); filtrar(true); }
 ...
 if(limpiar) limpiar.addEventListener('click',function(){ limpiarFiltros(true); filtrar(true); });
 tipo.addEventListener('change',function(){ filtrar(true); });
 if(prevPage) prevPage.addEventListener('click',function(){ if(paginaActual>1){paginaActual--;filtrar(false);} });
 if(nextPage) nextPage.addEventListener('click',function(){ const total=filasCoincidentes().length; const paginas=Math.ceil(total/PAGE_SIZE); if(paginaActual<paginas){paginaActual++;filtrar(false);} });
 actualizarContadores(); filtrar(true);
})();
}
...
function editUnitOptions(units, placeholder) {
  var html = '<option value="">' + (placeholder || 'Unidad') + '</option>';
...
function editReqRecalcRow(row) {
  if (!row) return;
  var qtyInput = row.querySelector('[name="cantidad[]"]');
  var stockInput = row.querySelector('[name="stock[]"]');
  ...
  var s = parseFloat((stockInput || {}).value || 0) || 0;
...
function editReqRowHasInformation(row) {
  ...
  return desc.trim() !== '' || qty !== '' || unit !== '' || (stock !== '' && Number(stock) !== 0) || just.trim() !== '' ||
...
function editAddReqRow(section) {
  ...
  tr.innerHTML = `
    ...
    <td class="nt-td-desc">
      ...
      <input type="hidden" name="item_key[]" value="${itemKey}">
      <textarea class="nt-auto-grow edit-auto" name="descripcion[]" rows="1" required placeholder="Descripción del material..."></textarea>
    </td>
    <td class="nt-td-cant"><input type="number" step="0.01" min="0.01" name="cantidad[]" value="" required placeholder="Cantidad" oninput="editReqRecalcRow(this.closest('tr'))"></td>
    <td class="nt-td-und"><select name="unidad[]" required>${editUnitOptions(window.LANR_REQ_UNITS || [], 'Seleccione')}</select></td>
    <td class="nt-td-stk"><input type="number" step="0.01" min="0" name="stock[]" value="0" oninput="editReqRecalcRow(this.closest('tr'))"></td>
    <td class="nt-td-buy"><input class="readonly-field edit-req-buy" readonly value=""></td>
  ...
  detailRow.innerHTML = `
    <td colspan="9"><div class="req-details-panel">
      ...
      <div class="req-details-grid">
        <div class="req-detail-field"><label title="Normal: sigue el proceso habitual. Prioritario: debe atenderse con preferencia. Urgente: requiere atención inmediata.">Prioridad</label><select name="prioridad_item[]" class="req-item-priority" title="Normal: sigue el proceso habitual. Prioritario: debe atenderse con preferencia. Urgente: requiere atención inmediata."><option value="Normal" selected>Normal</option><option value="Prioritario">Prioritario</option><option value="Urgente">Urgente</option></select><small>Normal por defecto. Cámbiela solo si corresponde.</small></div>
        ...
        <div class="req-detail-field req-detail-images"><label title="Opcional. Adjunte imágenes que ayuden a identificar el material.">Imágenes de referencia <span>(opcional)</span></label><label class="req-image-btn lanr-image-picker"><i class="fa-regular fa-images"></i><span>Adjuntar imágenes</span><small>JPG, PNG o WEBP · Máx. 5</small><input class="lanr-file-input-hidden" type="file" name="imagenes_${itemKey}" accept="image/jpeg,image/png,image/webp" multiple></label><span class="req-image-count">Sin imágenes</span><div class="req-image-preview" data-image-preview></div><small>JPG, PNG o WEBP. Puede agregar imágenes en varias selecciones hasta completar 5.</small></div>
      </div>
    </div></td>`;
...
function editAddReqRowAfter(btn) {
  ...
  tr.innerHTML = `
    <td class="nt-num"></td>
    <td class="nt-td-desc"><input type="hidden" name="seccion[]" value="${section}"><input type="hidden" name="item_key[]" value="${itemKey}"><textarea class="nt-auto-grow edit-auto" name="descripcion[]" rows="1" required placeholder="Descripción del material..."></textarea></td>
    <td class="nt-td-cant"><input type="number" step="0.01" min="0.01" name="cantidad[]" value="" required placeholder="Cantidad" oninput="editReqRecalcRow(this.closest('tr'))"></td>
    <td class="nt-td-und"><select name="unidad[]" required>${editUnitOptions(window.LANR_REQ_UNITS || [], 'Seleccione')}</select></td>
...
/* =========================================================
   CORRECCIÓN 29 — SIDEBAR RESPONSIVE / DRAWER
========================================================= */
(function () {
...
document.addEventListener("DOMContentLoaded", function () {
  document.querySelectorAll(".qm-quotation-form").forEach(function (form) {
    const selectAll = form.querySelector(".qm-select-all");
    const checks = Array.from(form.querySelectorAll(".qm-item-check"));
    const grand = form.querySelector(".qm-grand-total");

    function updateRow(check) {
      const row = check.closest(".qm-quote-row");
      const qty = row.querySelector(".qm-qty");
      const price = row.querySelector(".qm-price");
    ...
    function updateTotals() {
      ...
      checks.forEach(function (check) {
        const row = check.closest(".qm-quote-row");
        const qty = parseFloat(row.querySelector(".qm-qty").value || "0");
        const price = parseFloat(row.querySelector(".qm-price").value || "0");
        const subtotal = check.checked ? qty * price : 0;
        row.querySelector(".qm-line-total").textContent = "S/ " + subtotal.toFixed(2);
        total += subtotal;
      });
      if (grand) grand.textContent = "S/ " + total.toFixed(2);
    }
    ...
    checks.forEach(function (check) {
      check.addEventListener("change", function () {
        updateRow(check);
        if (selectAll) selectAll.checked = checks.length > 0 && checks.every(function (c) { return c.checked; });
      });
    });
    form.querySelectorAll(".qm-qty,.qm-price").forEach(function (input) {
      input.addEventListener("input", updateTotals);
    });
  });

  document.querySelectorAll(".qm-dispatch-form").forEach(function (form) {
    const radios = Array.from(form.querySelectorAll('input[name="guia_opcion"]'));
    const fields = form.querySelector(".qm-guide-fields");
    function toggleGuide() {
      const selected = radios.find(function (radio) { return radio.checked; });
      const pending = selected && selected.value === "pendiente";
      if (!fields) return;
      fields.querySelectorAll('input[name="guia_numero"],input[name="guia_fecha"],input[name="guia"]').forEach(function (input) {
        input.disabled = pending;
        if (pending) input.value = "";
      });
    }
    radios.forEach(function (radio) { radio.addEventListener("change", toggleGuide); });
    toggleGuide();
  });
});
